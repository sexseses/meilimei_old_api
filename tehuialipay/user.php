...
//testing
	...
	function getinfo($param='') {
		...
		$uid = $this->uid;
		$tmp = $this->db->query("SELECT users.ref_id,users.id,users.jifen,users.grade, users.username ,user.age,users.alias,users.email,users.phone,users.created,user_profile.* FROM users LEFT JOIN user_profile ON user_profile.user_id = users.id WHERE users.id = {$uid} LIMIT 1")->result_array();
		$result['data'] = $tmp[0];
	...
	*/
    ...
    function sendsmspy($param = '') {
        ...
        if ($phone = $this->input->POST('phone')) {
            $sn = rand(10000, 99999);
            $message = '【可叮】您的手机验证码是：' . $this->input->POST('code') . '退订回复TD';
            $this->sms->sendSMS(array (
                "{$phone}"
            ), $message);
            $result['notice'] = '发送成功';
    ...
    function sendsmsorderpy($param = '') {
        ...
        if ($phone = $this->input->POST('phone')) {
            $sn = rand(10000, 99999);
            $message = '【可叮】订单信息：' . $this->input->POST('message') . '退订回复TD';
            $this->sms->sendSMS(array (
                "{$phone}"
            ), $message);
            $result['notice'] = '发送成功';
