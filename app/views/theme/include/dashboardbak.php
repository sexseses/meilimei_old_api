<div class="Personal_center_left">
			 ...
			 $tmp = $this->db->get_where('wen_notify', array('user_id' => $this->wen_auth->get_user_id()),1 )->result(); }else if($roleid==2){echo '医师账户';}else{echo '医院账户';} ?> </h3>
                        <ul> <?php if($this->wen_auth->get_role_id()==3){ ?> <li <?php echo $this->uri->segment(2) == 'myyishi'?'class="on4"':''; ?>><a href="<?php echo base_url() ?>counselor/myyishi" class="item04">医师管理</a></li>  <?php } ?>
                        	...
                        	<li <?php echo ($this->uri->segment(2) == 'dashboard' || $this->uri->segment(2) == 'Fquestions' || $this->uri->segment(2) == 'underway' || $this->uri->segment(1) == 'question')?'class="on13"':''; ?>><a href="<?php echo base_url() ?>user/dashboard" class="item13">我的咨询<?php if($roleid==1 and $tmp[0]->new_answer >0) echo '<div class="newans"><div class="nlf"></div><em>'.$tmp[0]->new_answer.'</em><div class="nrf"></div></div>';elseif($roleid==2 and $newans >0) echo '<div class="newans"><div class="nlf"></div><em>'.$newans.'</em><div class="nrf"></div></div>';?></a></li>
