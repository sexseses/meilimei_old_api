...
<em style="font-weight:normal;padding-left:10px;font-style:normal;color:#999;">  <?php echo $result['total_rows'] ?>个</em>
        ...
        <dl class="clearfix">
            ...
            function Gcity($citylink,$arr){
				$i = 1;
				foreach($arr as $r){
					if($now_city == $r){ $class_action = "class='cur'";  }
					echo '<a '.$class_action.' href="'.$citylink.urlencode($r).'">'.$r.'</a>'.($i%17==0?'<br>':'');
					$i++;
				}
			} 
			$arr = array('北京','上海','杭州','广州','深圳','哈尔滨','呼和浩特','吉林','沈阳','石家庄','太原','天津','大连','长春','鞍山','丹东',
			'福州','合肥','济南','南昌','南京','宁波','青岛','厦门','苏州','徐州','温州','台州','金华','临沂','保定',
			'武汉','长沙','南宁','郑州','东莞','佛山','宜昌','重庆','成都','贵阳','昆明','兰州','乌鲁木齐','洛阳','唐山' ,'柳州'
			,'绵阳' ,'西宁' ,'常州' ,'包头' ,'烟台' ,'齐齐哈尔' ,'普洱' ,'大庆' , '潍坊' ,'秦皇岛' ,'淄博','珠海','无锡','阜阳','海口',
        ...
        <?php 
           ...
           <dd class="yiyuan"><ul> <li><a href="'.base_url().'jigouDetail/'.$r['user_id'].'"><img src="'.site_url().'public/images/yuyueon.png" /></a></li></ul></dd>
          </dl> ';
			}			
		 ?> 
