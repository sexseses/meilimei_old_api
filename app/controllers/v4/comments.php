    ...
    public function getCommentFloor($params = '')
    {
        ...
        if (($type = trim($this->input->get('type'))) and ($contentid = intval($this->input->get('contentid')))) {
            if (!($rs = $this->alicache->get($_SERVER['REQUEST_URI']))) {
                ...
                $fields = 'users.banned,users.alias as uname,users.city,users.age,users.phone,users.id as uid,c.id,c.contentid,c.comment,c.cTime,c.pid,c.pcid,c.new_reply,c.data,c.is_delete';
                $sql = "SELECT {$fields} FROM wen_comment as c LEFT JOIN users ON users.id = c.fuid ";
                $this->db->where('weibo_id', $contentid);
                $item = $this->db->get('wen_weibo')->result_array();
                ...
                foreach ($tmp as $r) {
                    ...
                    unset($r['data']);
                    $r['is_reply'] = '0';
                    $fields = 'users.banned,c.fuid,users.alias as uname,users.phone,c.id,c.contentid,c.touid,c.comment,c.cTime,c.is_delete,c.data';
                    $sql = "SELECT {$fields} FROM wen_comment as c LEFT JOIN users ON users.id = c.fuid ";
                    ...
                    if (!empty($tmps)) {
                        ...
                        foreach ($tmps as $item) {
                            ...
                            if (!empty($r['imgfile'])) {
                                ...
                                $r['haspic'] = '1';
                                $r['picture'] = $this->remote->getLocalImage($rtmp[0]['path']);
                                $r['height'] = isset($rtmp[0]['height']) ? $rtmp[0]['height'] : 200;
                                $r['width'] = isset($rtmp[0]['width']) ? $rtmp[0]['width'] : 200;
                            }
                            unset($item['data']);
    ...
    public function getCommentFloorImage($params = '')
    {
        ...
        if (($type = trim($this->input->get('type'))) and ($contentid = intval($this->input->get('contentid')))) {
            if (!($rs11 = $this->alicache->get($_SERVER['REQUEST_URI']))) {
                $page = intval($this->input->get('page')) - 1;
                $start = $page < 1 ? 0 : $page * 5;
                $fields = 'users.banned,users.age,users.city,users.alias as uname,users.phone,users.id as uid,c.id,c.contentid,c.comment,c.cTime,c.pid,c.pcid,c.new_reply,c.data,c.is_delete';
                $sql = "SELECT {$fields} FROM wen_comment as c LEFT JOIN users ON users.id = c.fuid ";
    ...
    public function Gcomments($param = '')
    {
        ...
        if (($type = trim($this->input->get('type'))) and ($contentid = intval($this->input->get('contentid')))) {
            //if (!($rs = $this->alicache->get($_SERVER['REQUEST_URI']))) {
                ...
                $page = intval($this->input->get('page')) - 1;
                $start = $page < 1 ? 0 : $page * 5;
                $fields = 'users.banned,users.age,users.city,users.alias as uname,users.phone,users.id as uid, users.jifen, users.city, users.age,c.id,c.contentid,c.comment,c.cTime,c.pid,c.pcid,c.new_reply,c.data,c.is_delete';
                $sql = "SELECT {$fields} FROM wen_comment as c LEFT JOIN users ON users.id = c.fuid ";
                if ($lastid) {
                ...
                foreach ($tmp as $r) {
                    ...
                    $fields = 'users.banned,c.fuid,users.alias as uname,users.phone, users.jifen, users.city, users.age,c.id,c.contentid,c.touid,c.comment,c.cTime,c.is_delete,c.data';
                    $sql = "SELECT {$fields} FROM wen_comment as c LEFT JOIN users ON users.id = c.fuid ";
                    $sql .= "WHERE type = '{$type}' and c.pid={$r['id']} and users.banned=0 order by c.id ASC";
                    $tmps = $this->db->query($sql)->result_array();
                    if (!empty($tmps)) {
