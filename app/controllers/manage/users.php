    ...
    public function userac() {
        ...
        if ($uid) {
            ...
            if ($banned == 0) {
                ...
                $splVars = array (
                    "{title}" => '账户通过审核',
                    "{content}" => '【美丽神器】恭喜您通过审核成为美神达人，将有20000美豆加入您的账号，双倍积分特权，欢迎成为最红达人！[如美豆加分有误请到APP首页快问小美处咨询]',
                    "{time}" => date('Y-m-d H:i',
                        time()), "{site_name}" => '美丽神器');
                ...
                if ($info[0]['phone'] != '') {
                    ...
                    $this->sms->sendSMS(array (
                        "{$info[0]['phone']}"
                    ), '【美丽神器】恭喜您通过审核成为美神达人，将有20000美豆加入您的账号，双倍积分特权，欢迎成为最红达人！[如美豆加分有误请到APP首页快问小美处咨询]');
                }
            } else {
                $this->db->limit(1);
                $this->db->where('uid',$uid );
