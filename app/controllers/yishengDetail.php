<?php
class yishengDetail extends CI_Controller {
	...
	public function index($param = '') {
		...
		if($this->input->post('uname')){
			...
			$data['userto'] = $uid;
			$data['name'] = strip_tags($this->input->post('uname'));
			...
			!$data['sex']&&$data['sex']='未知';
				$result['result'] = '亲爱的用户，你的' . $data['yuyueDate'] . '预约已提交，预约号:' . $data['sn'] . ' 等待医院/医师确认中;谢谢。退订回复TD';
		...
		$sql = "SELECT users.tconsult,users.systconsult,users.replys,users.sysreplys,users.alias as username,users.created,users.grade,users.sysgrade,user_profile.user_id,user_profile.sex,user_profile.company,user_profile.position,user_profile.department,user_profile.city,user_profile.introduce,user_profile.skilled";

		$csql = ' FROM users LEFT JOIN user_profile ';
		$csql .= ' ON user_profile.user_id = users.id  WHERE ';
		$csql .= ' users.role_id = 2 AND users.id = ' . $uid;
		...
		if (!empty ($tmp)) {
			...
			$data['message_element'] = "yisheng-detail";
			...
			$data['WEN_PAGE_TITLE'] =  $data['yisheng']['username'].'_'.$data['yisheng']['company'];
