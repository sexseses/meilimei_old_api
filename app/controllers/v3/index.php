...
class index extends MY_Controller
{
    ...
    public function getBanner($param = ''){
      ...
      if($this->input->get('tags') ){
         ...
         foreach($tmp as $r){
            ...
            if(in_array($city,unserialize($r['getLocalImage']))){
               $r['picture'] = $this->remote->getLocalImage(str_replace('upload/','',$r['picture']));
               $r['sharepic'] = $r['sharepic']==''?$r['picture']:$this->remote->getLocalImage(str_replace('upload/','',$r['sharepic']));;
               $result['data'][] = $r;
             }else{
                $r['picture'] = $this->remote->getLocalImage(str_replace('upload/','',$r['picture']));
                $r['sharepic'] = $r['sharepic']==''?$r['picture']:$this->remote->getLocalImage(str_replace('upload/','',$r['sharepic']));;
                        ...
                        /*if($i == 2){
                            ...
                            if(!empty($flashSale)){
                                foreach($flashSale as $item){
                                    $res[] = $item;//$this->getIndexFlashSale();
                                }
                            }
                        }*/
                        $rs = $this->getTags($i . "-1");
    ...
    private function getHN5($type = 1,$pageSize=10) {
        ...
        foreach ($tmp as $r) {
            ...
            $r['url'] = $this->remote->getLocalImage($url, $width);
	...
	private function getHN($type = 1,$pageSize=10) {
		...
		foreach ($tmp as $r) {
			...
			if($r['front_pic']){
                $r['url'] = $this->remote->getLocalImage($r['front_pic']);
            }else {
                $r['url'] = $this->remote->getLocalImage($url, $width);
            }
            if (!isset ($dtypd['pic']['height'])) {
    ...
    public function getIndexFlashSale(){
        ...
        $fields = "id,city,type,begin,end,lbanner,title,discount,vbuy";
        $condition = "";
        $limit = "limit 0,5";

        $fs_sql = "select $fields from flash_Sale as fs where 1=1 and display = 1 {$condition} {$limit}";
...
//                     $v['end']  = $hour.'小时';
//                 }
                $v['lbanner'] = $this->remote->getLocalImage($v['lbanner']);
                //print_r($v['flag']);die;
            }
    ...
    public function getIndexHotItems(){
        ...
        if($item_rs){
            foreach($item_rs as &$v){
                $v['pic'] = $this->remote->getLocalImage($v['pic']);
            }
            $result['data'] = $item_rs;
            $result['notice'] = '成功获取！';
        }else{
