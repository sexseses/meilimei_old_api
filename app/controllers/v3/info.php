...
class info extends MY_Controller {
    ...
    function softInfoXml(){
        ...
        if($os == 'ios'){
            ...
            echo json_encode(array('state' => '000', 'data' => array('version' => 33, 'versionName' => '3.22', 'description' => '3.22版本更新如下:
[Ver3.22/2015-09-24/13.12MB]
1.[系统] 兼容android5.0及+;
2.[特惠] 页面更新，购物更流畅;
3.[活动] 报名活动现在可以看到详情;
4.[优化] 个人中心订单部分调整.', 'path' => 'http://www.meilimei.com/m/meilishenqi.apk')));
        }
    }
 ...
 请点击确定在线升级，如果升级失败，请到http://www.meilimei.com/下载最新版本(安装包大小：10.38MB)','path'=>'http://www.meilimei.com/m/meilimei.apk')));
	...
	function faces($param = '') {
		...
		foreach ($files as $r) {
			...
			$result['data'][] = array (
				'name' => $name, 'url' => 'http://www.meilimei.com/images/faces/' . urlencode($r));
		}
		echo json_encode($result);
	}
	...
	function facesTop($param = '') {
		...
		$result['data'] = array ();
			...
			if($this->uid and ($page==0 OR $page==1) and $result['myScore'] = $this->getUloc()){
                  ...
                  $result['myScore']['pic'] = $this->remote->getLocalImage($result['myScore']['pic']);
			...
			foreach ($res as $k=>$r) {
				...
				$r['pic'] = $this->remote->getLocalImage($r['pic']);
