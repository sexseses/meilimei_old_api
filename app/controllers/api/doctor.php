...
class doctor extends CI_Controller {
	...
	public function jsearch(){
        ...
        if($jg){
        	 ...
        	 $SQL = "select alias as name from users as o LEFT JOIN user_profile as p ON u.user_id=p.id where u.role_id=2 and u.alias like '{$c}%' and p.company='{$jg}' limit 10";
        }
        if($c){
	...
	public function search($param = '') {
		...
		if ($this->auth->checktoken($param)) {
			...
			if ($this->input->get('company')) {
				$sql .= " user_profile.company = '" . $this->input->get('company') . "' AND ";
			}
			if (strstr($sql, 'AND')) {
	...
	public function questions($param = '') {
		...
		if ($this->auth->checktoken($param)) {
			if ($uid = intval($this->input->get('uid'))) {
				...
				$sql = "SELECT wen_questions.id,wen_questions.fuid,wen_questions.title, wen_questions.cdate,user_profile.sex,wen_questions.address,wen_weibo.type_data ";
				...
				foreach ($tmp as $row) {
                   ...
                   if(isset($tmps[1]['id'])){
                      ...
                      $row['pictures'][] = $this->remote->show($ptmp[0]['savepath'],128);
        	       }
				...
				//$sumn = $this->db->query("SELECT COUNT(wen_questions.id) as num FROM wen_questions LEFT JOIN wen_answer ON wen_questions.id = wen_answer.qid  WHERE wen_answer.uid = {$uid}")->result_array();

				//$this->db->query("update users SET tconsult={$sumn[0]['num']} where id={$uid} ");
			} else {
				$result['state'] = '012';
			}
