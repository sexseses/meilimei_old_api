	...
	private function CreateCoupon($order) {
        ...
        if($coupon_num != $order['quantity']) {
            for ($i = 0; $i < $order['quantity']; $i++) {
                ...
                $date = date('Y-m-d ','expire_time');
                /*$isCode = $this->getCode( $order['team_id'], $order['mobile']);
                if($isCode){
                    $message = "您已成功下单：{$order['title']}，优惠码：{$isCode}，请提前48小时拨打免费电话400-6677-245预约，到店请出示优惠码及支付尾款，感谢您使用【美丽神器】退订回复0000";
                }else{*/
                $message = "亲爱的美粉，您的订单已经成功下单:{$order['title']},特惠券号:{$id}," . "有效期至{$date}，需提前3天预约，预约咨询电话：400-667-7245。";
                /*}*/
                if ($order['mobile']) {
