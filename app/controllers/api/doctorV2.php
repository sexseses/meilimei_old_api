...
class doctorV2 extends CI_Controller {
	...
	public function getAns($param = '') {
		...
		if ($this->auth->checktoken($param)) {
			...
			if ($qid = $this->input->get('qid')) {
                ...
                //$result['data']['ans'] = $this->Gans($contentid);
			} else {
				$result['state'] = '012';
			}
	...
	private function getcomment($qid = 0, $uid = 0, $insert) {
		...
		//return "SELECT talk.*,users.alias as tFname FROM talk LEFT JOIN users ON users.id = talk.touid WHERE talk.qid = {$qid} AND (talk.fuid = {$uid} OR talk.touid = {$uid}) order by talk.id ASC";
        $result = array ();
