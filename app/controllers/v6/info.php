...
class info extends MY_Controller {
    ...
    function i($param = ''){
        ...
        $this->push->setTest();
        $push = array('type'=>'topic','id'=>1111,'page'=>1111);
        echo $this->push->send('This is the alert text',array(), 'f34dfdd1d09b59c3696e61fb7cf7f9aa9ed19c650b91a39129dab823f585471c');
    }
    ...
    function softInfoXml(){
        $os = $this->input->get('os')?$this->input->get('os'):'';
        if($os == 'ios'){
            echo json_encode(array('state'=>'000','v'=>'1.7.9', 'desc'=>'新版本APP发布了,快去下载吧～！','open'=>0, 'open2'=>1));
        }
        else {
            echo json_encode(array('state' => '000', 'data' => array('version' => 35, 'versionName' => '3.24', 'description' => '3.24版本更新如下:
[Ver3.24/2015-12-11/15.97MB]
1.[功能] 更新快问小美,退出APP后亦可收发消息;
2.[优化] 加快启动速度35%,内存占用减少30%,加快首页流畅度;
3.[优化] 优化多处细节,修复用户反馈bug.', 'path' => 'http://pic.meilimei.com.cn/upload/apk/meilishenqi.apk?version=35')));
        }
    }

    function softInfoIOS(){

    }
    function softInfoXmlForMeiliMei(){

        echo json_encode(array('state'=>'000','data'=>array('version'=>2,'versionName'=>'1.11','description'=>'1.优化二维码扫描识别速度
 2.加入连续按两次返回键退出app
 请点击确定在线升级，如果升级失败，请到http://www.meilimei.com/下载最新版本(安装包大小：10.38MB)','path'=>'http://www.meilimei.com/m/meilimei.apk')));
    ...
    function faces($param = '') {
        ...
        if(in_array($this->input->get('windowwidth'),array(320, 375))){
            $filetype = 2;
        }elseif(in_array($this->input->get('windowwidth'),array(414, '其他'))){
            $filetype = 3;
        }else{
            $filetype = 3;
        }

        $files = $this->getAllFiles($_SERVER['DOCUMENT_ROOT'].'/images/faces2/'.$filetype);
        ...
        foreach ($files as $r) {
            ...
            $encode = mb_detect_encoding($name, array("ASCII","UTF-8","GB2312","GBK"));


            if ($encode == "EUC-CN"){
                //echo $encode;

                $name = iconv("EUC-CN","UTF-8",$name);

                //echo $s;
            }
            $result['data'][] = array (
                'name' => $name, 'url' => 'http://www.meilimei.com/images/faces2/'.$filetype .'/'. urlencode($r), 'code'=> $encode);
        }
