...
class h5tehui extends MY_Controller {
            ...
            if (!empty ($tmp)) {
                ...
                if ($this->result['data']['image'] != '') {
                    if(strpos($this->result['data']['image'], 'http://pic.') === false && strpos($this->result['data']['image'], 'clouddn.com') === false) {
                        $images[] = 'http://tehui.meilimei.com/static/' . $$this->result['data']['image'];
                    }
                }
                if ($this->result['data']['image1'] != '') {
                    if(strpos($this->result['data']['image1'], 'http://pic.') === false && strpos($this->result['data']['image1'], 'clouddn.com') === false) {
                        $images[] = 'http://tehui.meilimei.com/static/' . $$this->result['data']['image1'];
                    }
                }
                if ($this->result['data']['image2'] != '') {
                    if(strpos($this->result['data']['image2'], 'http://pic.') === false && strpos($this->result['data']['image2'], 'clouddn.com') === false) {
                        $images[] = 'http://tehui.meilimei.com/static/' . $$this->result['data']['image2'];
                    }
                }
...
// 	        }
	...
	public function getOrderInfoById(){
		...
		if($pay_id){
			$sql = "select o.quantity,o.mobile as tel,t.* ,p.username,p.address from `order` as o left join team as t on o.team_id = t.id left join partner as p on t.partner_id = p.id where pay_id =  '{$pay_id}'";
			//$result['data']['sql'] = $sql;
			$tmp = $this->tehuiDB->query($sql)->row_array();
			$tmp['address'] = !empty($tmp['address'])?$tmp['address']:'';

			if (strpos($tmp['image'], 'http://pic.') === false && strpos($tmp['image'], 'clouddn.com') === false) {
				$tmp['image'] = 'http://tehui.meilimei.com/static/' . $tmp['image'];
			} else {
				if(strpos($tmp['image'], 'clouddn.com') === false){
					$tmp['image'] = $tmp['image'];
				}else{
					$tmp['image'] = $tmp['image'].'?imageView2/2/w/160/q/75/format/jpg';
				}
			}
			$result['data']['order_info'] = $tmp;
			$result['state'] = '100';
			$result['ustate'] = '100';
		}else{
			$result['notice'] = '参数不全！';
