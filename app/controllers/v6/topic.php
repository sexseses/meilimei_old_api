...
class topic extends MY_Controller
{
    ...
    function view($param = '')
    {
        ...
        if ($wid = $this->input->get('weibo_id')) {
            ...
            if (!empty ($tmp)) {
                ...
                $tmp[0]['username'] = !empty($rs[0]['alias']) ? $rs[0]['alias'] : $rs[0]['username'];
                $tmp[0]['username'] = !empty($tmp[0]['username']) ? $tmp[0]['username'] : '';

                if (preg_match("/^1[0-9]{10}$/", $tmp[0]['username'])) {
                    $tmp[0]['username'] = substr($tmp[0]['username'], 0, 4) . '****';
                }
                $result['data']['uname'] = $tmp[0]['username'];//$this->GName($tmp[0]['alias'],$tmp[0]['phone']);
                ...
                $result['data']['ctime'] = $this->getContentDateTime($tmp[0]['ctime']);
                //$result['data']['ctime'] = date('Y-m-d', $tmp[0]['ctime']);
                if ($tmp[0]['type_data']) {
                    $info = $this->emoji->mb_unserialize($tmp[0]['type_data']);
                ...
                if (!empty($tmp[0]['tags'])) {
                    ...
                    foreach ($row['tag'] as $item) {
                        if (!empty($item)) {
                            ...
                            if (intval($tmp[0]['event_id']) > 0) {
                                $arr['other'] = 4;
                                $arr['event_id'] = $tmp[0]['event_id'];
                            } else {
				...
				if(empty($ptmp[0]['imgfile'])) {
					$result['data']['shareurl'] = $this->remote->getLocalImage($arr_url[1], 160);
				}
				else
				{
					$result['data']['shareurl'] = $this->remote->getQiniuImage($ptmp[0]['imgfile'], 160);
				}*/
                if ($tmp[0]['type'] == 1 and !empty ($ptmp)) {
                    if (empty($tmp[0]['imgfile'])) {
                        $pictmp = $this->emoji->mb_unserialize($ptmp[0]['type_data']);
                        $this->db->where('id', $pictmp[1]['id']);
                        $this->db->from('wen_attach');
                        $this->db->select('savepath', 'imgfile');
                        $ptmp = $this->db->get()->result_array();
                        if (!empty ($ptmp)) {
                            ...
                            if (empty($ptmp[0]['imgfile'])) {
                                ...
                                $result['data']['url'] = $this->remote->getLocalImage($pictmp['pic']['savepath'], 640);
                                $result['data']['shareurl'] = $this->remote->getLocalImage($pictmp['pic']['savepath'], 150);
                            } else {
                                $result['data']['url'] = $this->remote->getQiniuImagewater($ptmp[0]['imgfile'], 640);
                                $result['data']['shareurl'] = $this->remote->getQiniuImage($ptmp[0]['imgfile'], 150);
                            }
                        }
                    } else {
                        $result['data']['url'] = $this->fileUrl . $tmp[0]['imgfile']."?imageView2/2/w/640/q/75/format/jpg%7Cwatermark/1/image/aHR0cDovLzd4a2RpOC5jb20xLnowLmdsYi5jbG91ZGRuLmNvbS93aXM3LnBuZw==/dissolve/40/gravity/Center";
                        $result['data']['shareurl'] = $this->remote->getQiniuImage($tmp[0]['imgfile'], 150);
                    }
                } elseif ($tmp[0]['type'] == 8) {
                    if (empty($tmp[0]['imgfile'])) {
                        $pictmp = $this->emoji->mb_unserialize($tmp[0]['type_data']);
                        $this->db->where('id', $pictmp[1]['id']);
                        $this->db->from('wen_attach');
                        $this->db->select('savepath', 'imgfile');
                        $ptmp = $this->db->get()->result_array();
                        ...
                        $result['data']['url'] = $this->remote->getQiniuImagewater($tmp[0]['imgfile'], 640);
                        $result['data']['shareurl'] = $this->remote->getQiniuImage($tmp[0]['imgfile'], 150);
                    }
                    ...
                    if (empty($tmp[0]['imgfile'])) {
                        ...
                        //$result['data']['url'] = $this->remote->getQiniuImage($tmp[0]['imgfile'], 640);
                        $result['data']['shareurl'] = $this->remote->getQiniuImage($tmp[0]['imgfile'], 150);
                    }
                    $result['data']['haspic'] = "1";
                    $result['data']['mutilPic'] = "1";
                    $result['data']['images'] = $this->plist($wid,640);
                ...
                $result['flashsale'] = array();

                if ($tmp[0]['fs_ids']) {
                    $result['flashsale'] = $this->getFlashSale($tmp[0]['fs_ids']);
                }
                //get extran links
                $result['extras'] = array();
                if ($tmp[0]['extra_ids']) {
                    ...
                    $tmp = $this->emoji->mb_unserialize($tmp[0]['product_data']);
                    foreach ($tmp as $r) {
    ...
    {
        if (intval($this->input->get('event_id')) > 0) {
            $this->uid = intval($this->input->get('event_id'));
            $this->eventDB = $this->load->database('event1', TRUE);
            $this->eventDB->insert('event_topic_detail', array('event_id' => intval($this->input->get('event_id')), 'topic_id' => 111, 'user_id' => $this->uid, 'top' => 0, 'dispaly' => 1, 'smscontext' => '', 'sms' => 'N', 'createtime' => time()));
        }
    }
    ...
    public function addTopicWithPics($param = '')
    {
        ...
        if ($this->uid) {
            ...
            if (strlen(strip_tags(trim($this->input->post('content')))) > 2) {
                ...
                $info = array();
                $info['title'] = $content;
                $datas['imgfile'] = $this->input->post('key1');
                $datas['type_data'] = $type_data = $this->emoji->mb_serialize($info);
                //get system info
                ...
                $datas['event_id'] = intval($this->input->post('event_id')) > 0 ? intval($this->input->post('event_id')) : 0;
                ...
                if (intval($this->input->post('event_id')) > 0) {
                    ...
                    $this->eventDB->where('id', intval($this->input->post('event_id')));
                    $this->eventDB->where('display', 1);
                    $tmp = $this->eventDB->get('event_topic')->result_array();
                    if (!empty($tmp)) {
                        $this->db->query("update users set jifen=jifen+{$tmp[0]['event_score']} where id='" . $this->uid . "'");
                        $result['event_score'] = "参加成功，奖励" . $tmp[0]['event_score'] . "美豆！";
                    }
                    $this->eventDB->insert('event_topic_detail', array('event_id' => intval($this->input->post('event_id')), 'topic_id' => $result['weibo_id'], 'user_id' => $this->uid, 'top' => 0, 'display' => 1, 'smscontext' => '', 'sms' => 'N', 'createtime' => time()));
                }
                ...
                if ($picnum = intval($this->input->post('has_pics'))) {
                    ...
                    for ($i = 1; $i <= $picnum; $i++) {
                        ...
                        if ($i == 1) {
                            $upstate = $this->addTopicPicAC($result['weibo_id'], $this->input->post('key' . $i), $this->input->post('upPicInfo' . $i), $type_data, true);
                            $result['debug' . $i] = $upstate;
                        } else {
                            $upstate = $this->addTopicPicAC($result['weibo_id'], $this->input->post('key' . $i), $this->input->post('upPicInfo' . $i), $type_data);
                            $result['debug' . $i] = $upstate;
                        }
                        if (!$upstate) {
                            break;
    ...
    public function addTopic($param = '')
    {
        ...
        if ($this->uid) {
            ...
            if (strlen(strip_tags(trim($this->input->post('content')))) > 2) {
                ...
                $datas['type_data'] = $this->emoji->mb_serialize($info);
                //get system info
                $head = $_SERVER['HTTP_USER_AGENT'];
                if ((stristr($head, 'iPhone') and !stristr($head, 'U;')) OR stristr($head, 'ipod')) {
                    $datas['wsource'] = 'IOS';
    ...
    public function topicSearch($param = '')
    {
        ...
        if ($this->input->get('page') and $kw = mysql_real_escape_string(trim($this->input->get('kw')))) {
            ...
            $this->db->select('users.jifen, users.age, users.city , users.alias as showname,wen_weibo.tags as tags,users.phone,wen_weibo.hot, wen_weibo.imgfile,wen_weibo.event_id,wen_weibo.hot_start,wen_weibo.hot_end,wen_weibo.top,wen_weibo.top_start,wen_weibo.top_end,wen_weibo.chosen,wen_weibo.chosen_start,wen_weibo.chosen_end,wen_weibo.weibo_id,wen_weibo.views,wen_weibo.comments as commentnums,wen_weibo.uid,wen_weibo.uid,wen_weibo.content,wen_weibo.ctime, wen_weibo.type_data');
            $this->db->from('wen_weibo');
            ...
            foreach ($tmp as $r) {
                ...
                $dtypd = $this->emoji->mb_unserialize($r['type_data']);
    ...
    private function addTopicPicAC($weibo_id = '', $picsrc = '', $picinfo = '', $type_data = '', $is_main = false)
    {
        if ($weibo_id and $picsrc) {
            ...
            $datas['name'] = uniqid() . rand(1000, 9999) . $ext;
            $datas['savepath'] = '';
            $ptmp = getimagesize($this->fileUrl . $picsrc);
            $datas['imgfile'] = $picsrc;
            $datas['userId'] = $this->uid;
            $datas['width'] = $ptmp[0];
            $datas['height'] = $ptmp[1];
            $b1 = $datas['width'] / 600;
            ...
            if ($wid = $this->input->post('weibo_id')) {
                if (isset ($_FILES['attachPic']['tmp_name']) && $_FILES['attachPic']['tmp_name']) {
                    ...
                    if (is_writable($target_path)) {
                        ...
                        $data['data'] = $this->emoji->mb_serialize($upicArr);
                    }
                }
                $data['type'] = 'topic';
                $data['status'] = 1;
                $data['contentid'] = $wid;
                $comment = $this->emoji2str($this->filter->filts($this->input->post('comment')));
    ...
    #美贴发布
    public function topicPublish($picid)
    {
        $data = array(
            'year' => date('Y', time()),
            'month' => date('m', time()),
            ...
            'topic_id' => $picid,
            'topic_selection' => $this->input->post('items'),
            'type' => 1
        );
        $this->db->insert('addup_topic', $data);
    }

    #美贴评论
    public function topicComment($picid, $item_name)
    {
        $data = array(
            'year' => date('Y', time()),
            'month' => date('m', time()),
        ...
        if ($this->input->get('page')) {
            ...
            if ($this->input->get('order') == 3) {
                ...
                if (!($rs = $this->alicache->get($_SERVER['REQUEST_URI']))) {
                    ...
                    $this->alicache->set($_SERVER['REQUEST_URI'], $this->emoji->mb_serialize($result));
                } else {
                    $result = array();
                    $result = $this->emoji->mb_unserialize($rs);
                }
            }
        } else {
            $result['state'] = '012';
    ...
    private function getHN4($type = 1, $pageSize = 10, $uid = 0)
    {
        ...
        foreach ($tmp as $r) {
            ...
            $r['ctime'] = $this->getContentDateTime($r['ctime']);
            $dtypd = $this->emoji->mb_unserialize($r['type_data']);
            isset ($dtypd['title']) && $r['content'] = $dtypd['title'];
            ...
            $r['newtime'] = $this->getContentDateTime($r['newtime']);
            if ($type == 1) {
    ...
    private function getHN($type = 1, $uid)
    {
        ...
        $this->db->limit(8, $offset);
        $this->db->join('users', 'users.id = wen_weibo.uid');
        $this->db->select('users.city, users.age,wen_weibo.weibo_id,wen_weibo.pageview,wen_weibo.event_id,wen_weibo.top,wen_weibo.top_start,wen_weibo.top_end,wen_weibo.newtime,wen_weibo.chosen,wen_weibo.chosen_start,wen_weibo.chosen_end,wen_weibo.hot,wen_weibo.hot_start,wen_weibo.hot_end,users.alias as uname,users.jifen, wen_weibo.views,wen_weibo.tags,wen_weibo.comments,wen_weibo.uid,wen_weibo.uid,wen_weibo.content,wen_weibo.ctime, wen_weibo.type_data');
        ...
        foreach ($tmp as $r) {
            ...
            $r['pageview'] = intval($r['views']);
            $rs = $this->Diary_model->get_user_by_username($r['uid']);
            $r['username'] = !empty($rs[0]['alias']) ? $rs[0]['alias'] : $rs[0]['username'];
            $r['username'] = !empty($r['username']) ? $r['username'] : '';
            if (preg_match("/^1[0-9]{10}$/", $r['username'])) {
                $r['username'] = substr($r['username'], 0, 4) . '****';
            }
            //$this->GName($row['alias'],$row['phone']);
            $r['uname'] = $r['username'];
            $r['zanNum'] = ($this->getZan($r['weibo_id']) > 0) ? $this->getZan($r['weibo_id']) : 0;
            if (intval($uid) > 0) {
                $iszan = $this->isAtZan($uid, $r['weibo_id']);
    ...
    public function GroupTopicListTop($param = '')
    {
        ...
        $sql = "SELECT w.group_start,w.group_end,w.event_id,w.comments,w.content, w.hot,w.hot_start,w.hot_end,w.top,w.top_start,w.top_end,w.chosen,w.chosen_start,w.chosen_end,w.uid, w.type_data,w.weibo_id,w.ctime,u.phone,u.email,u.alias,w.commentnums,w.tags ";
        ...
        if (!empty ($tmp)) {
            foreach ($tmp as $row) {
                ...
                $info = $this->emoji->mb_unserialize($row['type_data']);
                ...
                $row['username'] = isset($rs[0]['alias']) ? $rs[0]['alias'] : $rs[0]['username'];
    ...
    public function topicList1($param = '')
    {
        ...
        $sql = "SELECT w.group_start,w.group_end,w.comments,w.content, w.event_id,w.hot,w.hot_start,w.hot_end,w.top,w.top_start,w.top_end,w.chosen,w.chosen_start,w.chosen_end,w.uid, w.type_data,w.weibo_id,w.ctime,u.phone,u.email,u.alias,w.commentnums,w.tags ";
        $sql .= ' FROM wen_weibo as w LEFT JOIN users as u ON w.uid=u.id';
        ...
        if (!empty ($tmp)) {
            foreach ($tmp as $row) {
                ...
                $row['showname'] = $row['username'];//$this->GName($row['alias'],$row['phone']);
                //$row['showname'] = $this->GName($row['alias'],$row['phone']);
                if (isset ($info[1]['id']) OR isset ($info['pic'])) {
                    $row['haspic'] = 1;
                } else {
    ...
    public function topicList($param = '')
    {
        ...
        $sql = "SELECT u.jifen, u.daren,w.group_start,w.group_end,w.comments,w.content,w.newtime, w.event_id,w.hot,w.hot_start,w.hot_end,w.top,w.top_start,w.top_end,w.chosen,w.chosen_start,w.chosen_end,w.uid, w.type_data,w.weibo_id,w.ctime,u.phone,u.email,u.alias,w.commentnums,w.tags ";
        $sql .= ' FROM wen_weibo as w LEFT JOIN users as u ON w.uid=u.id';
        ...
        $sql .= " AND  w.isdel=0 ORDER BY w.newtime DESC ";
        if ($this->input->get('limit')) {
            $limit = $this->input->get('limit');
        } else {
            $limit = 10;
        ...
        if (!empty ($tmp)) {
            foreach ($tmp as $row) {
                ...
                $row['pageview'] = intval($row['pageview'])+ 102;
                ...
                $row['ctime'] = $this->getContentDateTime($row['ctime']);
    ...
    {
        $result['state'] = '000';
        $sql = "SELECT w.comments,w.content, w.uid, w.type_data,w.weibo_id, w.event_id,w.ctime,u.phone,u.email,u.alias,w.commentnums,w.tags,u.city, u.age, w.views,w.pageview ";
        $sql .= ' FROM wen_weibo as w LEFT JOIN users as u ON w.uid=u.id';
        $count = 0;
        $ctime = time(); //set publish time
        if ($this->input->get('uid')) {
        ...
        if (!empty ($tmp)) {
            foreach ($tmp as $row) {
                ...
                if ($this->uid) {
                    ...
                    $is = $this->Diary_model->isZan($this->uid, $row['nid']);
                    $item['isZan'] = $is ? 1 : 0;
                ...
                $row['showname'] = $row['username'];//$this->GName($row['alias'],$row['phone']);
                $row['alias'] = !empty($row['alias']) ? $row['alias'] : $row['showname'];
                if (isset ($info[1]['id']) OR isset ($info['pic'])) {
                    $row['haspic'] = 1;
                } else {
                    $row['haspic'] = 0;
    ...
    private function UdatePic($id, $size = array())
    {
        ...
        $this->db->where('weibo_id', $id);
        $tmp = $this->db->get('wen_weibo')->result_array();
        $type_data = $this->emoji->mb_unserialize($tmp[0]['type_data']);
        if ($type_data['pic']['savepath']) {
            $type_data['pic']['height'] = $size[1];
            $type_data['pic']['width'] = $size[0];
            $data['type_data'] = $this->emoji->mb_serialize($type_data);
            $this->db->where('weibo_id', $id);
            $this->db->limit(1);
            $this->db->update('wen_weibo', $data);
    ...
    public function comments($param = '')
    {
        ...
        if ($wid = $this->input->get('weibo_id')) {
            if (!($rs = $this->alicache->get($_SERVER['REQUEST_URI']))) {
                ...
                if (!empty ($tmp)) {
                    foreach ($tmp as $row) {
                        $row['comment'] = $this->str2emoji($row['comment']);
                        if ($row['data'] != '') {
                            $extra = $this->emoji->mb_unserialize($row['data']);
    ...
    //get flashsale list
    public function getflashSale($ids)
    {
        ...
        if (!empty($idarr)) {
            ...
            $rs = array();
            $fs_sql = "select id, banner, banner_key,end,begin from flash_sale where 1=1 and id in ($sec)";
            $fs_rs = $this->db->query($fs_sql)->result_array();


            if (!empty($fs_rs)) {

                foreach ($fs_rs as $item) {
                    if (empty($item['banner_key'])) {
                        $item['banner'] = $this->remote->getLocalImage($item['banner'], 640);
                    } else {
                        $item['banner'] = $this->remote->getQiniuImage($item['banner_key'], 640);
                    }
                    $item['begin'] = intval($item['begin'] - time());
                    $item['end'] = intval($item['end'] - time());
                    $item['flag'] = 0;
                    if ($item['end'] < 0) {
                        $item['flag'] = 5;
                    } elseif ($item['end'] <= 259200 && $item['end'] >= 0) {
                        $item['flag'] = 1;
                    } elseif ($item['begin'] >= -259200 && $item['begin'] <= 0) {
                        $item['flag'] = 3;
                    }
                    unset($item['banner_key']);
                    unset($item['end']);
                    unset($item['begin']);
                    $rs[] = $item;
                }
    ...
    private function getTehui($ids = '')
    {
        ...
        if (!empty($idarr)) {
            ...
            $sql = "select t.id,t.title,t.image,t.team_price,t.reser_price, t.now_number,t.market_price,t.delivery, t.mechanism from team  t where t.id in($sec)";
