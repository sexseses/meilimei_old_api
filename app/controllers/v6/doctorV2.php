...
class doctorV2 extends MY_Controller {
	...
	private function qdetail($param = '') {
		$result = null;
		if ($qid = $param) {
			$tmp = $this->db->query("SELECT id,fUid as fuid,position,title as content, title,address,description,cdate FROM wen_questions WHERE id ={$qid} ORDER BY id DESC  LIMIT 1 ")->result_array();
			if (!empty ($tmp[0])) {
				...
				if (!empty ($tmps) and $tmps[0]['type_data'] != '') {
					...
					$tmps = $this->db->query("SELECT savepath, imgfile FROM wen_attach WHERE id ={$pinfo[1]['id']} ORDER BY id DESC  LIMIT 1 ")->result_array();
                    if(empty($tmps[0]['imgfile'])) {
                        $tmp[0]['pic'] = $this->remote->getLocalImage($tmps[0]['savepath']);
                    }else{
                        $tmp[0]['pic'] = $this->remote->getQiniuImage($tmps[0]['imgfile']);
                    }
				}
	...
	private function getcomment($qid = 0, $uid = 0, $insert) {
		...
		foreach ($tmp as $row) {
            ...
            //$row['x'] = "SELECT talk.*,users.alias as tFname FROM talk LEFT JOIN users ON users.id = talk.touid WHERE talk.qid = {$qid} AND (talk.fuid = {$uid} OR talk.touid = {$uid}) order by talk.id ASC";
			unset ($row['qid']);
			unset ($row['data']);
