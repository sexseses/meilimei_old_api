...
class event extends MY_Controller
{
    ...
    public function getEnterList()
    {
        ...
        if (intval($event_id) > 0) {
            $start = intval($this->input->get('page')) > 0 ? intval($this->input->get('page')) : 1;
            $start = ($start - 1) * 10;
            $this->eventDB->where('event_id', $event_id);
            $this->eventDB->order_by('creattime', 'desc');
            $this->eventDB->limit(10, $start);
            $enter_uid = $this->eventDB->get('event_topic_enter')->result_array();

            $arr_uid = array();

            if (!empty($enter_uid)) {
                foreach ($enter_uid as $item) {
                    $arr_uid[] = (string)$item['user_id'];
                }

                $arr_user_id = implode(',', $arr_uid);

                $arr_user = $this->db->query("select u.id, u.username, u.alias, u.phone, u.jifen, up.sex, up.city from users u left join user_profile up on u.id = up.user_id where u.id in ({$arr_user_id}) order by field(u.id,{$arr_user_id})")->result_array();

                if (!empty($arr_user)) {
                    foreach ($arr_user as $item) {

                        $item['thumb'] = $this->remote->thumb($item['id'], '120');
                        $item['level'] = $this->getLevel($item['jifen']);
                        $item['age'] = $this->getAge($item['id']);
                        $item['username'] = !empty($item['alias']) ? $item['alias'] : $item['username'];
    ...
    public function getflashSaleByid()
    {
        ...
        $offset = ($start - 1) * $pageNum;
        $fs_sql = "select * from flash_sale where 1=1 and id = ?";
        $fs_pr_sql = "select * from flash_sale_tehui where 1=1 and fs_id = ? and display=1 order by id DESC limit $offset,$pageNum";
        $fs_rs = $this->db->query($fs_sql, array($id))->row_array();
        ...
        if (!empty($fs_rs)) {
            ...
            $tehui_fields = 't.newversion,t.pre_number,t.p_store,t.id, t.mechanism,t.city_id,t.czone,t.user_id,t.summary,t.title, t.image,t.team_price, t.now_number,t.market_price,t.delivery,t.reser_price,t.deposit';
            ...
            if (!empty($fs_pr_rs)) {
                ...
                foreach ($fs_pr_rs as $fsv) {
                    ...
                    $tehui_condition .= " and t.team_type='normal' and t.end_time >= '{$time}'  and display = 1";
                    if (!empty ($city_id)) {
                    ...
                    if (!empty($tehui_info)) {
                        if (!strstr($tehui_info['image'], "http://pic") && strpos($tehui_info['image'], 'clouddn.com') === false) {
                            $tehui_info['image'] = 'http://tehui.meilimei.com/static/' . $tehui_info['image'] . '?' . $randpic;
                        }

                        if ($tehui_info['mechanism']) {
                            $this->db->where('userid', $tehui_info['mechanism']);
                        ...
                        //session 唯一标示付
                        $sid = $tehui_info['id'];
                        $tehui_info['now_number'] = $this->getNowNumber($tehui_info['id']);
                        ...
                        if ($tehui_info['reser_price']) {
                            $tehui_info['reser_price'] = $tehui_info['reser_price'];
                        } else {
                            $tehui_info['reser_price'] = "0.00";
                        }

                        if ($tehui_info['deposit']) {
                            $tehui_info['deposit'] = $tehui_info['deposit'];
                        } else {
                            $tehui_info['deposit'] = "0.00";
                        }
            ...
            if (empty($fs_rs['banner_key'])) {
                $fs_rs['banner'] = $this->remote->getLocalImage($fs_rs['lbanner'], 640);
            } else {
            ...
            //web=android,else ios, *^&^*
            if ($web == 'web') {
                $result['data'] = $fs_rs;
                $result['data']['tehui_list'] = $tehui_area_arr;
            } else {
                $result['data'] = $fs_rs;
                $result['tehui_list'] = $tehui_area_arr;
            }
    ...
    private function gdetail($content, $title)
    {
        ...
        <style>
            ...
            .mainc p {margin:0px; padding:0px;}
...
/*            .mainc img{
        ...
        if (intval($event_id) > 0) {
            ...
            if (!empty($tmp)) {
                foreach ($tmp as $item) {
                    ...
                    $item['event_content'] = $this->gdetail($item['event_content'], 'tmp');
    ...
    private function topicList($event_id = 0)
    {
        ...
        $this->eventDB->where('display', 1);
        $this->eventDB->limit(10, $start);
        $this->eventDB->order_by('top', 'desc');
        $this->eventDB->order_by('createtime', 'desc');
        ...
        if (!empty($utopic)) {
            ...
            $sql .= " AND ctime<={$ctime} and w.isdel=0  ORDER BY FIELD(w.weibo_id,{$topicid}) ";
