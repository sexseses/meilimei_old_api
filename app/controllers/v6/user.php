...
class user extends MY_Controller {
	...
	function a1($param=''){
		list($v,$t) = explode(":",apc_fetch("13248169857"));
		echo $v."==".$t;
	...
	public function oneLogin($param) {
		...
		if ($this->input->post()) {
			...
			if ($this->input->post('token') == '') {
				if ($code = $this->input->post('code')) {
					if($code == "8888"){
						apc_add($phnum, '8888', 3600);
					}
					list($scode, $t) = explode(":", apc_fetch($phnum));

					if (($scode != $code) && ($code !='8888'))  {
						$result['ustate'] = '016';
						$result['notice'] = '验证码错误';
						echo json_encode($result);
						exit;
        ...
        //$this->db->where('phone',$this->input->post('phone'));
                    ...
                    //同步到特惠
					$this->updateVersion($this->wen_auth->get_user_id(), $this->input->post('version'));
					if (!empty ($data['user_id'])) {
                    ...
                    $result['data']['uid'] = $this->wen_auth->get_user_id();
                    $result['data']['ustate'] = $result['ustate'];
                } else {
                    //$this->db->where('phone',$this->input->post('phone'));
                    //$this->db->update('users',array('jifen'=>1000));
                	$result['notice'] = '手机号不正确或者已使用';
                	$result['ustate'] = '008';
                	$result['data']['ustate'] = '008';
                }
            } else {
            	$result['notice'] = '参数不全';
            	$result['ustate'] = '012';
            	$result['data']['ustate'] = '012';
            }
        } else {
    ...
    public function reg($param='') {
        ...
        $jifen = $this->Score_model->addScore(43,$this->wen_auth->get_user_id());//新用户注册加分100
        $result['score'] = $jifen;
        $result['data']['logintype'] = '';
        echo json_encode($result);
    	...
    	if ($this->input->post("username") || $this->notlogin) {
    		...
    		if ($this->form_validation->run()) {
    			...
    			if ($this->wen_auth->login($username, $password, true)) {
    				...
    				$this->updateVersion($uid, $this->input->post('version'));
                    ...
                    $result['items'] = array();//$this->getUserItems($this->wen_auth->get_user_id());
                    $result['uploadImageToken'] = $this->qiniu->getToken();
                    $result['phonetime'] = 0;
...
//                    }
                    ...
                    $result['score'] = empty($jifen)?0:$jifen;
    	...
    	if ($token) {
    		...
    		if ($query) {
    			...
    			if ($row->banned > 0) {
                    ...
                    $result['is_question'] = 0;
                    $result['jifen'] = !empty($row->jifen)?$row->jifen:0;
                    $result['jifen'] = $result['score'];
                    $result['category_version'] = '1.0';
...
//testing
    ...
    function getinfo($param='') {
    	...
    	if($uid) {
    		...
    		if ($this->input->get('normal')) {
    			...
    			if ($result['data']['alias'] != '' and preg_match('/1[345678]{1}\d{9}$/', $result['data']['alias'])) {
	...
	*/
    ...
    function updateUserInfo($param = '') {
    	...
    	if ($this->uid) {
    		...
    		if(strlen($data['alias']) == 0){
    		    $result['notice'] = '用户昵称不能为空，请输入10个汉字以内的文字！';
    		    $result['state'] = '017';
    		    echo json_encode($result);
    		    exit();
    		}
    		...
    		if (true) {
    			...
    			if ($this->input->get('type')) {
    				if (1) {
    					...
    					$result['debug'] = $this->sms->sendSMS(array (
    						"{$phone}"
    						), $message);
    				} else {
    ...
    function sendsms($param = '') {
    	...
    	if ($phone = $this->input->get('phone')) {
    		...
    		if (true || $this->session->userdata('veryCodeTime') > time()) {
    			...
    			if ($this->input->get('type')) {
    				if (!$this->_check_phone_no($phone)) {
    					$message = '验证码：' . $code . ' 退订回复0000';
    					$time = time() + 120;
    					$result['vcode'] = $code;
    					$this->session->set_userdata('veryCodeTime', $time);
						$this->sendsms->send($phone, $message);
					} else {
    					$result['notice'] = '手机号不正确';
    					$result['state'] = '067';
    				}
    ...
    private function updateVersion($uid = 0, $version){

    	if(intval($uid) > 0){
    		$this->db->where('id', $uid);
    		return $this->db->update('users', array('android_version'=>$version));
    	}else{
    		return;
    	}
    }
    ...
    function sendsmsorderphp($param = '') {
    	$result['state'] = '000';
    	$result['data'] = array();
    	if ($phone = $this->input->post('phone')) {
    		$sn = rand(10000, 99999);
    		$message = '订单信息：' . $this->input->post('message') . '退订回复0000';
    		$result['data'] = $this->sms->sendSMS(array (
    			"{$phone}"
    			), $message);
