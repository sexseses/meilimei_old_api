...
class index extends MY_Controller
{
     ...
     * 开机大屏广告
     */
    public function startBigAd(){
        $url_rs = $this->db->query("select * from startdb ")->row_array();
        //$data['data'] = $url_rs;
        return  $url_rs;
    }
    ...
    public function getHomeBody(){
        ...
        //if(!($rs = $this->alicache->get($_SERVER['REQUEST_URI']))) {
            if ($page == 1) {
                ...
                $result['data']['ad'] = $ad;
                $result['data']['piazza'] = $this->getHN(2,3,0);
                $result['data']['activities'] = $this->getTags("activities", 3,'',0);
            }
    ...
    public function getIndexAd(){
        ...
        $rs = $this->getTags("index-1",10,'',0);
        
        if(!empty($rs)) {
            foreach($rs as $rs_v){
                $res[] = $rs_v;//$this->getIndexFlashSale();
            }
        }
    ...
    public function getpiazza(){
        $result['data'] = array();
        $page = $this->input->get('page') ? $this->input->get('page') : 1;
        $result['data']  = $this->getHN(2,5,$page);
    ...
    public function getNewHomeBody(){
        ...
        if ($page == 1) {
            ...
            $start_ad = $this->getTags("startindex", 1,'',0);

            if($start_ad){
                $result['data']['start_ad'] = $start_ad[0];
            }else{
                $result['data']['start_ad'] =  0 ;
            }


            $result['data']['ad'] = $ad;
            $result['data']['piazza'] = $this->getHN(2,3);
            $result['data']['activities'] = $this->getTags("activities", 3,'',0);
        }
        if($page){
            $result['data']['flash_sale'] = $this->getIndexAd();
        }

        $result['data']['reality'] = $this->getHN2();
    ...
    private function getTags($tags = "middleBanner",$pageSize = 10,$type='',$limit = 0){
        $now_time = time();
        if($limit ==  0){
            $this->db->limit($pageSize);
        }else{
            $this->db->limit($pageSize,($limit-1)*$pageSize);
        }

        $this->db->select('title,id,spcid,subtype,picture,picture_key,sharepic,area,tehuiid,flashid,event_id,bdate,edate,is_img');
        ...
        if($type == 'hot'){
            $fuzha_sql = "select * from (select title,id,spcid,subtype,picture,picture_key,sharepic,area,tehuiid,flashid,event_id,bdate,edate,is_img from apple where bdate <= {$now_time} and edate >= {$now_time} and  tags != 'startindex' and (subtype = 3 or subtype = 4)  order by edate  ) as t1  union all select * from (select title,id,spcid,subtype,picture,picture_key,sharepic,area,tehuiid,flashid,event_id,bdate,edate,is_img from apple where edate <= {$now_time} and (subtype = 3 or subtype = 4)  order by edate  ) as t2  limit $start,$pageSize";
            $tmp = $this->db->query($fuzha_sql)->result_array();
        }else if($type == 'notice' ){
            $notic_sql = "select title,id,spcid,subtype,picture,picture_key,sharepic,area,tehuiid,flashid,event_id,bdate,edate,is_img from apple where bdate > {$now_time} and (subtype = 3 or subtype = 4)  order by bdate  limit $start,$pageSize";
            $tmp = $this->db->query($notic_sql)->result_array();
...
//echo $this->db->last_query();
    ...
    public function getAdBanner($tag = 'adbanner', $limit = 3){
        ...
        foreach( $result['data'] as &$result_v){
            $img_arr = getimagesize($result_v['picture']);
            if($img_arr[0] != $img_arr[1]){
                $result_v['picture'] = "http://7xkdi8.com1.z0.glb.clouddn.com/empty.png";
            }
        }
    ...
    public function getNewAdBanner(){
        $result['data'] =  array();
        $page = $this->input->get('page')?$this->input->get('page'):1;
        $offset = 5;
        $tag = $this->input->get('tag')?$this->input->get('tag'):'adbanner';
        ...
        $result['data'] = $this->getTags($tag,$offset,$type,$page);

        foreach( $result['data'] as &$result_v){
            $result_v['over'] = 0;
            if($result_v['edate'] < time()){
                $result_v['over'] = '1';
            }
            $result_v['bdate'] =  abs($result_v['bdate']-time());
            $bday = floor($result_v['bdate']/(3600*24));
            $bsecond = $result_v['bdate']%(3600*24);//除去整天之后剩余的时间
            $bhour = floor($second/3600);
            $bsecond = $second%3600;//除去整小时之后剩余的时间
            $bminute = floor($second/60);
            $bsecond = $second%60;//除去整分钟之后剩余的时间


            $result_v['full_begin_day'] = $bday.'天'.$bhour.'小时'.$bminute.'分'.$bsecond.'秒';
            $result_v['begin_day'] = $bday.'天';
            $result_v['begin_houer'] = $bhour.'小时';
            $result_v['begin_minute'] = $bminute.'分';

            if($bday){
                $result_v['begin_remain'] = '开始倒计时'.$bday.'天';
            }
            if($bday == 0 && $bhour){
                $result_v['begin_remain'] = '开始倒计时'.$bhour.'小时';
            }
            if($bday == 0 && $bhour == 0 && $bminute){
                $result_v['begin_remain'] = '即将开始';
            }



            $result_v['edate'] =  abs($result_v['edate']-time());
            $day = floor($result_v['edate']/(3600*24));
            $second = $result_v['edate']%(3600*24);//除去整天之后剩余的时间
            ...
            $result_v['full_end_day'] = $day.'天'.$hour.'小时'.$minute.'分'.$second.'秒';
            $result_v['end_day'] = $day.'天';
            $result_v['end_houer'] = $hour.'小时';
            $result_v['end_minute'] = $minute.'分';

            if($day){
                $result_v['end_remain'] = '还剩'.$day.'天结束';
            }
            if($day == 0 && $hour ){
                $result_v['end_remain'] = '还剩'.$hour.'小时结束';
            }
            if($day == 0 && $hour == 0 && $minute){
                $result_v['end_remain'] = '即将结束';
            }
       ...
       if($type == ""){
           $result['data'][] = array(
               ...
               'subtype' => "1",
               'end_remain'  => "即将结束",
               'picture' => "http://7xkdi8.com1.z0.glb.clouddn.com/events_all.png",
               'picture_key'=> "",
               'sharepic'=> "http=>//pic.meilimei.com.cn/picture/0_auto",
               'area'=> "b=>0;",
               'tehuiid'=> "0",
               'flashid'=> "0",
               'event_id'=> "20",
               ...
               'type'=> 1,
               'over'=> 0,
               'full_begin_day'=> "3天0小时0分57秒",
               'begin_day'=> "0天",
               'begin_houer'=> "4小时",
               'begin_minute'=> "22分",
               'begin_remain'=> "开始倒计时3天",
               'full_end_day'=> "0天4小时22分57秒",
               'end_day'=> "0天",
               'end_houer'=> "4小时",
               'end_minute'=> "22分"
           );
        ...
        if(!empty($result['data'])){
			...
			foreach($result['data'] as $item){
                ...
                if(empty($item['imgfile'])){
                    ...
                    $item['imgurl'] = $this->remote->getQiniuImageWater($item['imgfile'], 360);
				...
				$item['thumb'] = $this->profilepic($item['uid']);
				$item['zanNum'] = ($this->Diary_model->getZan($item['nid'])>0)?$this->Diary_model->getZan($item['nid']):0;
				$item['created_at'] = $this->getContentDateTime($item['created_at']);
                $item['diary_items'] = array();
                $tmp = $this->Diary_model->getItemsPrice($item['nid']);
	...
	//get new and hot topics
	private function getHN2($type = 1,$pageSize=5,$j = 1) {
	    ...
	    if(!empty($result['data'])){
	        ...
	        foreach($result['data'] as $item){
	            if(empty($item['imgfile'])){
                    $aa = $item['imgurl'];
                    $item['imgurl'] =   $this->remote->getLocalImage($aa, 360);
	                $item['savepath_big'] =  $this->remote->getLocalImage($aa, 640);
                    $item['savepath_big_test'] = "这是一家垃圾公司，都是坑别来";
	            }else{
                    $item['imgurl'] =  $this->remote->getQiniuImageWater($item['imgfile'], 360);

	                $item['savepath_big'] =  $this->remote->getQiniuImageWater($item['imgfile'], 640);
	            }
	            //
	            $rs = $this->Diary_model->get_user_by_username($item['uid']);
	            $item['username'] = isset($rs[0]['alias']) ? $rs[0]['alias'] : $rs[0]['username'];
	...
	private function getHN($type = 1,$pageSize=3,$limit=0,$newvesion = 1) {
        $this->load->model('Diary_model');
		$this->db->from('wen_weibo');
		$this->db->where('wen_weibo.isdel', 0);
        $this->db->where('wen_weibo.top', 1);
		...
		$this->db->join('users', 'users.id = wen_weibo.uid');
		$this->db->select('wen_weibo.front_q_pic,wen_weibo.weibo_id,wen_weibo.imgfile,wen_weibo.tags,wen_weibo.front_pic,users.alias as uname,wen_weibo.front_title,wen_weibo.views as vote,wen_weibo.comments,wen_weibo.uid,wen_weibo.uid,wen_weibo.content,wen_weibo.ctime, wen_weibo.type_data');


        if($pageSize == 9 || $newvesion == 1) {
		...
		foreach ($tmp as $r) {
            ...
            if (!empty($r['tags'])) {
                ...
                foreach ($r['tag'] as $item) {
                    if ($item) {
                        ...
                        $arr['other'] = $this->Diary_model->isItemLevel($itemid, 3);//fixed ios tag redirect bug..
                        $r['tagss'][] = $arr;
                    }
                }
            }
            ...
            }else if ($r['front_q_pic']){
                $r['url'] = $this->remote->getQiniuImage($r['front_q_pic']);
            }else{
                if(empty($r['imgfile'])) {
                    $r['url'] = $this->remote->getLocalImage($url);
                }else{
    ...
    public function info($param = ''){
      ...
      if($id = intval($this->input->get('id'))){
         ...
         if($tmp[0]['id'] == 163 || $tmp[0]['id'] == 246|| $tmp[0]['id'] == 425)
             $result['data']['pic'] = 1;
         else
            $result['data']['pic'] = 0;
         $result['state'] = '000';
    ...
    public function getIndexFlashSale(){
        ...
        $thistime= time();
        $fields = "fs.id,city,type,begin,end,lbanner,title,discount,vbuy,lbanner_key,banner, banner_key";
        $condition = " and fs.begin <= {$thistime} and fs.end >= {$thistime}";
        
        //$limit = "limit 0,2";
        
        $index_sql = "select * from flash_sale_index as ifs where 1 and display =1";
        $fs_ad_rs = $this->db->query($index_sql)->result_array();

        if(count($fs_ad_rs)>0){ 
            $fs_sql = "select $fields from flash_sale as fs left join flash_sale_index as ifs on fs.id = ifs.fs_id where 1=1 and ifs.display = 1 {$condition} "; 
        }else{
            $fs_sql = "select $fields from flash_sale as fs where 1=1 and id in(123,121) and display = 1 {$condition} ";
        }
