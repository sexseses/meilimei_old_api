...
class diary extends MY_Controller {
				...
				foreach($tmpDayDiaryList as $k=>$item) {
					...
					foreach ($tmp as $k => $citem) {
						...
						if(empty($citem['imgfile'])) {
							...
							$citem['sImgUrl'] = $this->remote->getQiniuImagewater($citem['imgfile'], 360);
							$citem['imgurl'] = $this->remote->getQiniuImagewater($citem['imgfile'], 480);
						}
				...
				foreach($tmpDayDiaryList as $k=>$item) {
					...
					foreach ($tmp as $key => $citem) {
						...
						if(empty($citem['imgfile'])) {
							...
							$citem['savepath_big'] = $this->remote->getQiniuImagewater($surl, 480);
			...
			if(!empty($rs)){
				...
				foreach($rs as $item){
					...
					if(empty($item['imgfile'])){
                        ...
                        $surl = $item['imgfile'];
						$item['imgurl'] = $this->remote->getQiniuImagewater($surl, 480);
						$item['shareurl'] = $this->remote->getQiniuImage($surl, 150);
					}
	...
	public function saveComment($param = ''){
		...
		if($nid && $content && $this->uid){
			...
			$clientid = $this->Users_model->getClientID($nid, 1);
		                    ...
		                    if($this->getNextCID($cid, 'diary') > 0){
		                        $lastid  = $this->getNextCID($cid , 'diary') ;
		                    }else{
		                        $lastid = "";
		                    }
			...
			if($this->uid != $tmpnote['uid']) {
				if (!empty($clientid)) {
					...
					$d = $this->igttui->sendMessage($clientid[0]['clientid'], "diary:" . $nid . ":" . $result['page'] .':'.$lastid. ':' . $content);
					$result['d'] = $d;
				} else {
			...
			if($pcid > 0){
				...
				$tmp = $this->Diary_model->getCommentDetail($cid);
				
	

								if(preg_match("/^1[0-9]{10}$/",$tmp['fromusername'])){
									$tmp['fromusername'] = $this->getAlias($tmp['fromusername']);//substr($it['fromusername'],0,4).'****';
								}
								if(preg_match("/^1[0-9]{10}$/",$it['tousername'])){
									$tmp['tousername'] = $this->getAlias($tmp['tousername']);//substr($it['tousername'],0,4).'****';
								}
								if(empty($tmp['imgfile'])) {

									$tmp['imgurl'] = !empty($tmp['imgurl']) ? $this->remote->getLocalImage($tmp['imgurl']) : '';
									$ptmpc = getimagesize($tmp['imgurl']);
									$tmp['width'] = isset($ptmpc[0]) ? $ptmpc[0] : 0;
									$tmp['height'] = isset($ptmpc[1]) ? $ptmpc[1] : 0;
								}else{
									$tmp['imgurl'] = !empty($tmp['imgfile']) ? $this->remote->getQiniuImage($tmp['imgfile']): '';
									$ptmpc = getimagesize($it['imgurl']);
									$tmp['width'] = isset($ptmpc[0]) ? $ptmpc[0] : 0;
									$tmp['height'] = isset($ptmpc[1]) ? $ptmpc[1] : 0;
								}

								if ($tmp['created_at'] > time() - 3600) {
									$tmp['created_at'] = intval((time() - $tmp['created_at']) / 60) . '分钟前';
								} else {
									$tmp['created_at'] = date('Y-m-d', $tmp['created_at']);
								}
				$tmp['isreply'] = 0;
				...
				$tmp['thumb'] = $this->remote->thumb($tmp['fromuid'],105);
				$temp = $this->Diary_model->get_user_by_username($tmp['fromuid']);
				$username = !empty($temp[0]['alias'])?$temp[0]['alias']:$temp[0]['username'];
				$tmp['username'] = $username;
				if(preg_match("/^1[0-9]{10}$/",$tmp['username'])){

					$tmp['username'] = $this->getAlias($tmp['username']);//substr($item['username'],0,4).'****';
				}

				if(preg_match("/^1[0-9]{10}$/",$tmp['fromusername'])){
					$tmp['fromusername'] = $this->getAlias($tmp['fromusername']);//substr($item['fromusername'],0,4).'****';
				}
				$tmp ['basicinfo'] = $this->getBasicInfo($temp[0]);
				$tmp['level'] = $this->getLevel($temp[0]['jifen']);
				$tmp['age'] = $this->getAge($tmp['fromuid']);

				if(empty($item['imgfile'])) {
					$tmp['imgurl'] = !empty($tmp['imgurl']) ? $this->remote->getLocalImage($tmp['imgurl']) : '';
					$ptmp = getimagesize($item['imgurl']);
					$tmp['width'] = isset($ptmp[0]) ? $ptmp[0] : 0;
					$tmp['height'] = isset($ptmp[1]) ? $ptmp[1] : 0;
				}else{

					$tmp['imgurl'] = $this->remote->getQiniuImage($tmp['imgfile']);
					$ptmp = getimagesize($item['imgurl']);
					$tmp['width'] = isset($ptmp[0]) ? $ptmp[0] : 0;
					$tmp['height'] = isset($ptmp[1]) ? $ptmp[1] : 0;
				}
				...
				if(isset($this->uid)) {
					$is = $this->Diary_model->isZan($this->uid, $tmp['cid'], 'diary_comments');
					$tmp['isZan'] = $is?1:0;
				}else{
					$tmp['isZan'] = 0;
				}
				$tmp['zanNum'] = ($this->Diary_model->getZan($tmp['cid'], 'diary_comments')>0)?$this->Diary_model->getZan($tmp['cid'], 'diary_comments'):0;
	...
	public function getAndroidComments($param = ''){
		...
		if(intval($nid) > 0){
			$tmp = $this->Diary_model->getAndroidCommentList($nid, $lastid, 5, $sort);
			...
			if(!empty($tmp)){
				...
				foreach($tmp as $item){
					...
					$item['created_at'] = $this->getCommentDateTime($item['created_at']);
					if(isset($this->uid)) {
						$is = $this->Diary_model->isZan($this->uid, $item['cid'], 'diary_comments');
						$item['isZan'] = $is?1:0;
					}else{
					...
					//if($type == 1){
						...
						if(!empty($item['reply'])){
							...
							foreach($item['reply'] as $it){
								...
								$it['created_at'] = $this->getCommentDateTime($it['created_at']);
								$reply[] = $it;
							}
							$item['reply'] = array();
							$item['reply'] = $reply;
		...
		if($uid){
			...
			if(!empty($temp)){
				foreach($temp as $item){
					...
					$item['signin_jifen'] = 10;//用户签到积分改为10
		...
		if($this->uid){
			...
			if(!empty($tmp)){
				foreach($tmp as $item){
					...
					$item['content'] = isset($rs[0]['content'])?$rs[0]['content']:'';
					if(empty($item['content'])){
						$r = $this->emoji->mb_unserialize($rs[0]['type_data']);
						$item['content'] = isset($r['title'])?$r['title']:'';
						if(empty($rs[0]['imgfile'])) {
							$item['pic'] = $this->remote->getLocalImage($r['pic']['savepath']);
						}else{
					...
					$item['yourcomment'] = $this->emoji->str2emoji($item['comment']);
					$user = $this->Diary_model->get_user_by_username($item['fuid']);
					$item['myname'] = isset($userInfo[0]['alias'])?$userInfo[0]['alias']:$userInfo[0]['username'];
					$item['yourname'] = isset($user[0]['alias'])?$user[0]['alias']:$user[0]['username'];
					$item['yourname'] = $item['yourname']?$item['yourname']:'';
					...
					$item['created_at'] = $this->getCommentDateTime($item['cTime']);

					if($this->getNextCID($item['itemid'], 'xxx') > 0){
						$item['lastid']  = $this->getNextCID($item['itemid'], 'xxx') ;
					}else{
						$item['lastid'] = "";
					}
					unset($item['comment']);
				...
				foreach($tmp as $item){
					...
					if (!empty($rstmp)) {
						...
						foreach ($rstmp as $it) {
							...
							if($this->getNextCID($item['cid'],'diary') > 0){
								$item['lastid'] = $this->getNextCID($item['cid'],'diary');
							}else{
								$item['lastid'] = "";
							}
							if(isset($this->uid)) {
	...
	public function diaryList($pageSize=10) {
		...
		if(!empty($tmp)){
			...
			foreach($tmp as $item){
				...
				if(empty($item['imgfile'])) {
					$aab  = $item['imgurl'];
					$item['imgurl'] = $this->remote->getLocalImage($aab);
					$item['savepath_big'] = $this->remote->getLocalImage($aab,640);
				}else{
					$item['imgurl'] = $this->remote->getQiniuImage($item['imgfile'],640);
				...
				if(!empty($category)){
					...
					if(!empty($category[0]['imgfile'])){
						...
						$item['operation_imgurl'] = $this->remote->getQiniuImagewater($category[0]['imgfile']);
					}else {
					...
					if(!empty($item['imgfile'])){
						...
						$item['operation_imgurl'] = $this->remote->getQiniuImagewater($item['imgfile']);
					}else {
	...
	//get points data
	private function GpointtTmp($picid){
		...
		if(!empty($tmp)){
			...
			foreach($tmp as $item){
				if(empty($item['imgfile'])) {
					...
					$item['imgurl'] = $this->remote->getQiniuImagewater($item['imgfile']);
				}
				$rs = $this->Diary_model->get_user_by_username($item['uid']);
				$item['username'] = isset($rs[0]['alias']) ? $rs[0]['alias'] : $rs[0]['username'];
				$item['username'] = isset($item['username'])?$item['username']:'';
