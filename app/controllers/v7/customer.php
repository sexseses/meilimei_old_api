...
class customer extends MY_Controller {
	...
	public function sendpic($param='') {
		...
		//if (isset ($_FILES['attachPic']['tmp_name']) && $_FILES['attachPic']['tmp_name'] && $data['q_id'] != 0) {      //传到咨询端图片
            ...
            $datas['savepath'] = '';
            $datas['imgfile'] = $this->input->get('key1');//date('Y').'/' . date('m').'/' . date('d').'/' . $datas['name'];
	...
	public function yuyue($param='') {
        ...
        $result['debug'] = strtotime($this->input->post('yuyueDate'));
		echo json_encode($result);
	}
    ...
    public function yuyue_cancel(){
        ...
        if ($this->uid) {
            $this->db->where('id',$id);
            $result['data'] = $this->db->update('yuyue',array('state'=>4));
        }else{
            $result['state'] = '012';
            $result['notcie'] = '用户未登录！';
        }
	...
	public function yuyueList($param=''){
		...
		if ($this->uid) {
			...
			if($this->wen_auth->get_role_id()==1){
				$info = $this->db->query("SELECT yuyue.id,yuyue.name as alias,yuyue.cdate,yuyue.phone,yuyue.userto,yuyue.age,yuyue.sex,yuyue.keshi,yuyue.state,yuyue.keshi,yuyue.yuyueDate, yuyue.extraDay, yuyue.sn,yuyue.remark FROM yuyue   WHERE yuyue.state !=4 and yuyue.userby = {$this->uid} order by cdate desc LIMIT $page,10 ")->result_array();
			}else{
				$info = $this->db->query("SELECT yuyue.id,yuyue.name as alias,yuyue.cdate,yuyue.userto,yuyue.phone,yuyue.age,yuyue.sex,yuyue.keshi,yuyue.state,yuyue.keshi,yuyue.yuyueDate, yuyue.extraDay,yuyue.sn,yuyue.remark FROM yuyue   WHERE yuyue.state !=4 and yuyue.userto = {$this->uid} order by cdate desc LIMIT $page,10 ")->result_array();
			}
            if(!empty($info)) {
                foreach ($info as $row) {
                    switch ($row['state']) {
                    ...
                    $rs = $this->getDoctorDetail($row['userto']);
                    if(isset($rs[0]['id'])) {
                        $rss = $this->getPosition($row['userto']);
                        $row['doctorName'] = $rs[0]['alias'] ? $rs[0]['alias'] : $rs[0]['username'];
                        $row['grade'] = $rs[0]['grade']; //五星评分
                        $row['alias'] = $row['alias'];

                        $row['thumb'] = $this->remote->thumb($rs[0]['id']);
                        $row['city'] = isset($row['city'])?$row['city']:'';
                        if(isset($row['age'])){
                            $row['age'] = $this->getAge($row['id']);
                        }else{
                            $row['age'] = '';
                        }
                        $row['position'] = $rss[0]['position'];

                        $row['yuyueDate'] = date('Y-m-d', $row['yuyueDate']) . ' ' . $row['extraDay'];
    ...
    public function getDoctor($param = ""){
        ...
        if(intval($uid) > 0) {
            $sql = "SELECT users.utags, users.age,users.city,users.tconsult,users.systconsult,users.replys,users.sysreplys,users.alias as username,users.created,users.voteNum,users.grade,users.sysgrade,users.sysvotenum,users.suggested,users.verify,user_profile.user_id,user_profile.user_id,user_profile.sex,user_profile.company,user_profile.position,user_profile.department,user_profile.city,user_profile.introduce,user_profile.skilled";
            $sql .= ' FROM users LEFT JOIN user_profile ';
            $sql .= ' ON user_profile.user_id = users.id  WHERE users.id=' . $uid;
            $tmp = $this->db->query($sql)->result_array();

            if(!empty($tmp)){
                foreach($tmp as $item){
                    $item['thumb'] = $this->remote->thumb($item['user_id']);
                    $item['city'] = isset($item['city'])?$item['city']:'';
                    $rs = $this->Diary_model->get_user_by_username($item['user_id']);
                    $item ['basicinfo'] = $this->getBasicInfo($rs[0]);
                    if(isset($item['age'])){
                        $item['age'] = $this->getAge($item['user_id']);
                    }else{
                        $item['age'] = '';
                    }
                    $result['data'] = $item;
	...
	public function getPosition($uid = 0){
