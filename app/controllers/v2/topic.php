...
class topic extends MY_Controller {
		...
		if ($wid = $this->input->get('weibo_id')) {
			if(!($rs = $this->alicache->get($_SERVER['REQUEST_URI']))){
				...
				if (!empty ($tmp)) {
					...
					'type' => 'topic', 'contentid' => $tmp[0]['weibo_id']);
					...
					if ($tmp[0]['type'] == 1 and !empty ($ptmp)) {
						...
						if (!empty ($ptmp)) {
                            ...
                            if(intval($arr_url[1]) >= 3 && intval(date('Y')) <= $arr_url[0]){
                                ...
                                $result['data']['url'] = $this->remote->show800($url, $width);
                            }else{
                                if(isset($arr_url[1])){
    ...
    private function getZan($contentid) {
        ...
        return $this->db->get_where('wen_zan', $where_condition)->num_rows();
    }
	//is repeate
	...
	public function addTopicWithPics($param = '') {
		...
		if ($this->uid) {
            ...
            $ttag = str_replace('热门', '', trim($this->input->post('new_items')));
	...
	public function topicSearch($param = '') {
		...
		if ($this->input->get('page') and $kw = mysql_real_escape_string(trim($this->input->get('kw')))) {
			...
			$this->db->select('users.alias as showname,wen_weibo.tags as tags,users.phone,wen_weibo.hot,wen_weibo.hot_start,wen_weibo.hot_end,wen_weibo.top,wen_weibo.top_start,wen_weibo.top_end,wen_weibo.chosen,wen_weibo.chosen_start,wen_weibo.chosen_end,wen_weibo.weibo_id,wen_weibo.views,wen_weibo.comments as commentnums,wen_weibo.uid,wen_weibo.uid,wen_weibo.content,wen_weibo.ctime, wen_weibo.type_data');
			$this->db->from('wen_weibo');
...
//        print_r($rr);
    ...
    private function getHN($type = 1, $uid) {
        ...
        $this->db->join('users', 'users.id = wen_weibo.uid');
        //	$this->db->join('wen_comment', 'wen_comment.contentid = wen_weibo.weibo_id');
        $this->db->select('wen_weibo.weibo_id,wen_weibo.top,wen_weibo.top_start,wen_weibo.top_end,wen_weibo.chosen,wen_weibo.chosen_start,wen_weibo.chosen_end,wen_weibo.hot,wen_weibo.hot_start,wen_weibo.hot_end,users.alias as uname,wen_weibo.views,wen_weibo.tags,wen_weibo.comments,wen_weibo.uid,wen_weibo.uid,wen_weibo.content,wen_weibo.ctime, wen_weibo.type_data');
	...
	public function topicList($param = '') {
		...
		$sql = "SELECT w.comments,w.content, w.hot,w.hot_start,w.hot_end,w.top,w.top_start,w.top_end,w.chosen,w.chosen_start,w.chosen_end,w.uid, w.type_data,w.weibo_id,w.ctime,u.phone,u.email,u.alias,w.commentnums,w.tags ";
