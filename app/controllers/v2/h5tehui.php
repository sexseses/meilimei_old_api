        ...
        if(count($rs)<=0){
            ...
            if (!empty ($tmp)) {
                ...
                if ($this->result['data']['image'] != '') {
                    if(!strstr($this->result['data']['image'],"http://pic")){
                ...
                if ($this->result['data']['image1'] != '') {
                    if(!strstr($this->result['data']['image1'],"http://pic")){
                        $images[] = $this->result['data']['image1'] = 'http://tehui.meilimei.com/static/' . $this->result['data']['image'];
                    }else{
                        $images[] = $this->result['data']['image'];
                    }  
                }
                if ($this->result['data']['image2'] != '') {
                    if(!strstr($this->result['data']['image2'],"http://pic")){
                        $images[] = $this->result['data']['image2'] = 'http://tehui.meilimei.com/static/' . $this->result['data']['image'];
                    }else{
                        $images[] = $this->result['data']['image'];
                    }
                }
...
// 	        }
	...
	public function getOrderInfoById(){
		...
		if($pay_id){
			...
			//$result['data']['sql'] = $sql;
			$result['data']['order_info'] = $this->tehuiDB->query($sql)->row_array();
			$result['state'] = '100';
			$result['ustate'] = '100';
		}else{
			$result['notice'] = '参数不全！';
