...
// 	    }
	...
	public function getTehuiDetailById(){
        ...
        $this->result = $this->alicache->get($t_id);
        echo '<pre>';
        print_r($this->result);
        if(!($this->result = $this->alicache->get($t_id))) {
            echo "1";
            ...
            if(strpos($this->result['data']['image'], 'http://pic.') === 0 && strpos($this->result['data']['image'], 'clouddn.com') === 0) {
        ...
        if ($this->result['data']['image1'] != '') {
            if(strpos($this->result['data']['image1'], 'http://pic.') === 0  && strpos($this->result['data']['image1'], 'clouddn.com') === 0) {
        ...
        if ($this->result['data']['image2'] != '') {
            if(strpos($this->result['data']['image2'], 'http://pic.') === 0  && strpos($this->result['data']['image2'], 'clouddn.com') === 0) {
            ...
            $this->alicache->set($_SERVER['REQUEST_URI'],serialize($this->result));
        }else{
	...
	public function getUserCoupons($param = '') {
	    ...
	    if ($this->uid) {
	        ...
	        foreach ($tmp as $r) {
	        	...
	        	if(strpos($r['image'], 'http://pic.') === 0 && strpos($r['image'], 'clouddn.com') === 0) {
           			$r['imgurl'] = 'http://tehui.meilimei.com/static/' . $r['image'];
            	}
...
// 	        }
	...
	public function order($param = '') {
		...
		if (($quantity = $this->input->post('quantity')) and $id = intval($this->input->post('id'))) {
			...
			if (!empty ($team)) {
				$team = $team[0];
				if($team['begin_time'] >= $time || $team['end_time'] <= $time){
				    $result['state'] = '001';
				    $result['notice'] = '产品未上架，或已下架';
				    echo json_encode($result);
				    exit;
