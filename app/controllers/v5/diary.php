...
//                     item['userBackgroundImg']=$this->remote->show320($this->Background_Model->getUserBackground($uid,1));
                    unset($item['alias']);
                    $result['data'] = $item;
                }
            }
                    ...
                    if(isset($item['type']) && $item['type'] == 'topic') {
                        ...
                        if (empty($item['content'])) {
                        	...
                        	if(empty($rs[0]['imgfile'])) {
                                ...
                                $item['pic'] = $this->fileUrl.$rs[0]['imgfile'];
                            }
                        }
    ...
    public function diaryList($pageSize=10) {
        ...
        if(!empty($tmp)){
            ...
            foreach($tmp as $item){
                ...
                $arr_url = explode('/',$item['imgurl']);
                $url = '';
                $url = str_replace('/' . $arr_url[1] . '/', '/' . $arr_url[1] . 'x640/', $item['imgurl']);
