...
class index extends MY_Controller
{
    ...
    public function getBanner($param = ''){
      ...
      if($this->input->get('tags') ){
         ...
         $this->db->select('title, id,spcid,subtype,picture,sharepic,area,tehuiid,flashid,event_id');
         $tmp = $this->db->get('apple')->result_array();
         $city = $this->input->get('city');

         $result['data'] =  array();
         foreach($tmp as $r){
             $r['event_title'] = $r['title'];
    ...
    private function getTags($tags = "middleBanner",$pageSize = 10){
        ...
        foreach($tmp as $r){
            ...
            if(intval($r['event_id']) > 0) {
                $r['title'] = $this->getEventTitle($r['event_id']);
            }
            if($tags != "activities"){
    ...
    private function getEventTitle($event_id){

        $this->eventDB->where('id',$event_id);
        $row = $this->eventDB->get('event_topic')->row_array();
	...
	private function getHN1($type = 1,$pageSize=5,$j = 1) {
        ...
        if(!empty($result['data'])){
			...
			foreach($result['data'] as $item){
                ...
                $item['imgurl'] = $this->remote->show320($url);
                ...
                if(!empty($category)){
                    ...
                    $arr_url = explode('/',$category[0]['imgurl']);
                    $url = '';
                    $url = str_replace('/' . $arr_url[1] . '/', '/' . $arr_url[1] . 'x640/', $category[0]['imgurl']);
                    $item['operation_imgurl'] = $this->remote->show320($url);
    ...
    public function getIndexFlashSale(){
        ...
        $thistime= time();
        $fields = "id,city,type,begin,end,lbanner,title,discount,vbuy,lbanner_key";
        $condition = " and fs.begin <= {$thistime} and fs.end >= {$thistime}";
        
        $limit = "limit 0,3";

        $fs_sql = "select $fields from flash_sale as fs where 1=1 and id in(42,64,62,57) and display = 1 {$condition} {$limit}";
