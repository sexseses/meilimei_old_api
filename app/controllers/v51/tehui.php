                ...
                if ($this->result['data']['mechanism']) {
                	...
                	if ($m_id) {
                		...
                		if ($company) {
                			...
                			$this->result['data']['mechanism'][false]['grade'] = $tmp_user['grade'];
                			$this->result['data']['mechanism'][false]['thumb'] = $this->profilepic($m_id, 2);
                			$abc = false;
                			$this->result['data']['mechanism'][false]['ablum'] = $this->ablum($m_id, $abc);
                			$this->result['data']['mechanism'][false]['verify'] = $tmp_user['verify'];
                			$this->result['data']['mechanism'][false]['suggested'] = $tmp_user['suggested'];
                
                			$this->result['data']['mechanism'][false]['physician'] = rand(10, 20);
                ...
                if($product_id){
                    ...
                    if($r['r_product']){
                        foreach($r['r_product'] as $rpv){
                            ...
                            $this->tehuiDB->join('partner', 'partner.id=team.partner_id', 'left');$this->tehuiDB->select('team.*,partner.comment_good,partner.comment_none,partner.comment_bad,partner.address, partner.longlat,partner.phone as partner_phone,partner.title as partner_name');
...
//echo $this->tehuiDB->last_query();die;
            ...
            if(!$rs[false]['mechanism']){
            	$rs[false]['mechanism'] = $this->result['data']['mechanism'];
            }
            if(!$rs[false]['physician']){
            	$rs[false]['physician'] = $this->result['data']['physician'];
            }
...
// 	            }
	...
	public function getNewTehuiList($param = '') {
	    ...
	    if ($page) {
	        $time = time();
	        $start = ($page -1) * 10;
	        $fields = 't.city_ids,t.newversion,t.pre_number,t.p_store,t.id,t.user_id,t.summary,t.title,t.image,t.team_price, t.now_number,t.market_price,t.delivery,t.reser_price,t.deposit,t.mechanism';
	        $condition = "t.team_type='normal' and t.begin_time <= '{$time}' and t.end_time >= '{$time}' and flashSale = 0 ";
...
// 	        }
	        ...
	        $order = ' t.sort_order DESC ';
	        $condition .= ' and t.relation = 1 and t.display = 1';
