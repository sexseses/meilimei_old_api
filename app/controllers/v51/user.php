			...
			if (($phnum != '' || $email != '') && $device_sn != '') {
				//if ($this->form_validation->run()) {
                   ...
                   $result['username'] = $username;
                    ...
                    //$this->db->query("update users set username=concat('美粉',id), alias=concat('美粉',id) where alias like '%meilizhensuo%' and id=".$this->wen_auth->get_user_id());
				    //$this->db->like('alias','meilizhensuo');
                    //$this->db->update('users',array('username'=>"concat('美粉',id)",'alias'=>"concat('美粉',id)"));
        ...
        if ($this->input->post("username") || $this->notlogin) {
            ...
            if ($this->form_validation->run()) {
                ...
                if ($this->wen_auth->login($username, $password, true)) {
                    ...
                    $result['items'] = array(array('id'=>680, 'surl'=>'http://pic.meilimei.com.cn/upload/2013%2F1387259140.png', 'name'=> '磨骨瘦脸','colors' => '0'),array('id'=>684, 'surl'=>'http://pic.meilimei.com.cn/upload/2013%2F1387259140.png', 'name'=> '磨骨瘦脸','colors' => '0'),array('id'=>685, 'surl'=>'http://pic.meilimei.com.cn/upload/2013%2F1387249511.png', 'name'=> '磨骨瘦脸','colors' => '0'));//$this->getUserItems($this->wen_auth->get_user_id());
                    $result['phonetime'] = 0;
                    if(count($result['items']) < 3){
                        unset($result['items']);
                        $this->addFollowUser($this->wen_auth->get_user_id());
                        $result['items'] = array(array('id'=>680, 'surl'=>'http://pic.meilimei.com.cn/upload/2013%2F1387259140.png', 'name'=> '磨骨瘦脸','colors' => '0'),array('id'=>684, 'surl'=>'http://pic.meilimei.com.cn/upload/2013%2F1387259140.png', 'name'=> '磨骨瘦脸','colors' => '0'),array('id'=>685, 'surl'=>'http://pic.meilimei.com.cn/upload/2013%2F1387249511.png', 'name'=> '磨骨瘦脸','colors' => '0'));
                    }
                    $result['messageNotReadCount'] = $this->getMessagesNotReadCount($this->wen_auth->get_user_id());
...
//testing
    ...
    private function getfans($uid,$type=0, $page=1){
        ...
        if ($uid) {
            ...
            $result =  array();
            foreach($tmp as $r){
                $r['thumb'] = $this->profilepic($r['fid'],2);
