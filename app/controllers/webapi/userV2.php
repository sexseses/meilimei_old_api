...
class userV2 extends CI_Controller {
	...
	function myQuestion($param = '') {
		...
		if ($this->auth->checktoken($param)) {
			...
			$this->db->select('wen_questions.title,wen_questions.id,wen_questions.cdate,wen_questions.state,wen_questions.new_answer');
