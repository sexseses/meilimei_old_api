...
//topic list
    public function index($page = '') {
        ...
        $fields = "w.weibo_id,w.tags,w.wsource,w.comments,w.q_id,w.content,w.uid,w.ctime,w.type_data,users.alias,users.phone,users.email,users.state";
    ...
    public function comments($page = '') {
        ...
        if ($this->input->get('uid')) {
            $condition .= "  AND wen_comment.fuid = {$this->input->get('uid')}";
    ...
    public function detail($param = '', $page = '') {
        ...
        $data['results'] = $this->db->query("SELECT id,comment,fuid,alias FROM wen_comment WHERE contentid = {$param} AND type='topic' AND  is_delete=0  ORDER BY id DESC  LIMIT $offset , $per_page")->result();      //评论结果
    ...
    public function edit($param = '') {
        if ($this->input->post('title')) {
            ...
            if ($_FILES['frontpic']['tmp_name']) {
                ...
                if ($this->remote->cp($_FILES['frontpic']['tmp_name'], $upruduct['name'], $upruduct['savepath'] . $upruduct['name'], true)) {
                    $udata['front_pic'] = $upruduct['savepath'] . $upruduct['name'];
                }
  ...
  /*  public function addpoint($param = ''){
    ...
    public function Suser() {
        if ($t = trim($this->input->get('term'))) {
            if (!$this->input->get('type') OR $this->input->get('type') == 'topic') {
            ...
            } elseif ($this->input->get('type') == 'ans') {
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            if ($_FILES['frontpic']['tmp_name']) {
                ...
                $upruduct['savepath'] = date('Y') . '/' . date('m') . '/' . date('d') . '/';
                if ($this->remote->cp($_FILES['frontpic']['tmp_name'], $upruduct['name'], $upruduct['savepath'] . $upruduct['name'], true)) {
                    $datas['front_pic'] = $upruduct['savepath'] . $upruduct['name'];
            ...
            $picorder = $this->input->post('picture_order');
            $is_vedio = $this->input->post('is_vedio');
            $picinfobak = $this->input->post('picture_info');
            ...
            if ($picorder) { //上传图片
                $tmpdir = date('Y') . '/' . date('m') . '/' . date('d') . '/';
                $i = 0;
                foreach ($picorder as $key => $r) {

                    if($is_vedio[$i]){
                        $addpl['attachId'] = $weibo_id;
                        $addpl['name'] = uniqid(time() . rand(1000, 99999), false) . '.mp4';
                        $addpl['savepath'] = $vedio[$i];
                        $addpl['userId'] = $this->uid;
                        $addpl['cTime'] = time();
                        $addpl['type'] = 'mp4';
                        $addpl['privacy'] = 0;
                        $addpl['order'] = $picorder[$i];
                        $addpl['info'] = strip_tags($picinfobak[$i]);
                        ...
                        if ($i == 0) {
                            ...
                            $ptmp = getimagesize($_FILES['picture']['tmp_name'][1]);



                            if ($this->remote->cp($_FILES['picture']['tmp_name'][1], $addpl['name'], $addpl['savepath'], array('width' => 600, 'height' => 800), true)) {
                                ...
                                $addpl['privacy'] = 0;
                                $addpl['order'] = $picorder[$i];
                                $addpl['info'] = strip_tags($picinfo[$i]);
                                $this->common->insertData('topic_pics', $addpl);
                            }
