...
/*
  ...
  */  }
     ...
     * @return mixed
     */
    public function getCommentList($nid = 0, $offset=0, $limit=10, $type=0){
        ...
        $this->db->join('users u','n.fromuid=u.id');
        $this->db->limit($limit, $offset);
        $this->db->order_by('n.cid','asc');
        return $this->db->get()->result_array();
    }

    public function getCommentDetail($cid= 0){
    ...
    public function getAndroidCommentList($nid = 0, $lastid = 0, $limit=5, $sort = "desc"){

        $this->db->select('n.* ');
        $this->db->where('n.nid',$nid);
        $this->db->where('n.pcid',0);
        $this->db->from('note_comment n');
        $this->db->join('users up','n.fromuid = up.id');
        ...
        if (strtolower($sort) == "asc") {
            ...
            if($lastid > 0){
                ...
                $sql = "select *From (SELECT n.* FROM note_comment as n LEFT JOIN users ON users.id = n.fromuid ";
                $sql .= "WHERE  n.cid < {$lastid} and n.nid={$nid} and n.pcid=0  order by n.cid DESC limit 5) as tmp order by cid ASC";
               return $this->db->query($sql)->result_array();
                ...
                $this->db->join('users up','n.fromuid = up.id');
                if($total > 5){
                    $this->db->limit($limit,  abs($total - 5));
                }else{
                    $this->db->limit($limit);
                }
                $this->db->order_by('n.cid','asc');
...
*/
    ...
    public function getDiaryMyFollowCommentsList($uid = 0, $offset=0, $limit = 10){
        ...
        $this->db->select('cid,nid, content, fromuid, fromusername, created_at, is_read');
        $this->db->where('touid', $uid);
