...
class wen_Auth {
	...
	*/
	...
	function _set_last_ip_and_last_login($user_id) {
		...
		if ($this->auth->config->item('WEN_login_record_time')) {
	...
	*/
	...
	function get_username() {
		return $this->auth->session->userdata('WEN_username');
	}
		...
		if (!empty ($login) AND !empty ($password)) {
		 $get_user_function = 'get_login';
			...
			if ($query = $this->auth->Users_model-> $get_user_function ($login) AND $query->num_rows() == 1) {
				...
				if ($row->banned > 0) {// Check if user is banned or not
					$this->_banned = TRUE;
	...
	function register($username, $password, $email,$phone='', $device_sn = '', $ref_id = '', $role_id=1,$coupon_code = '', $created = '', $user_id = '',$sendmail = falses) {
		// Load Models
		if($role_id==1){
			$banned = 0;
		}else{
		...
		)),'rev_phone'=>$re_phone, 'email' => $email, 'ref_id' => $ref_id,'role_id'=>$role_id, 'device_sn' => $device_sn, 'phone' => $phone, 'timezone' => 'UP8', 'coupon_code' => $coupon_code, 'last_ip' => $this->auth->input->ip_address(), 'created' => $created, 'banned' => $banned);
		...
		if ($this->auth->config->item('WEN_email_activation')) {
			...
			// Trigger event
			$this->auth->wen_auth_event->user_activated($insert);
		}
