...
class event extends MY_Controller {
    ...
    public function getflashSaleByid(){
        ...
        $fs_pr_sql = "select * from flash_sale_tehui where 1=1 and fs_id = ? and display=1 order by level DESC limit $offset,100";
        $fs_rs = $this->db->query($fs_sql,array($id))->row_array();
                ...
                if(!empty($city)){
                    ...
                    if(!empty($fs_pr_rs)){
                        ...
                        foreach($fs_pr_rs as $fsv){
                            ...
                            if(!empty($tehui_info)){
                                if(!strstr($tehui_info['image'],"http://pic")  and strpos($tehui_info['image'], 'clouddn.com') === false){
                                    $tehui_info['image'] = 'http://tehui.meilimei.com/static/' . $tehui_info['image'].'?'.$randpic;
                                }
