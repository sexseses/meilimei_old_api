...
class topic extends CI_Controller {
		...
		if ( $this->auth->checktoken($param)) {
			if ($wid = $this->input->get('weibo_id')) {
				if(!($rs = $this->alicache->get($_SERVER['REQUEST_URI']))){
					...
					$this->db->select('user_profile.city,users.alias,wen_weibo.extra_ids,wen_weibo.tags,wen_weibo.product_data,wen_weibo.uid,wen_weibo.type,wen_weibo.ctime,wen_weibo.q_id,wen_weibo.content,wen_weibo.type_data,wen_weibo.commentnums,wen_weibo.favnum,wen_weibo.views,wen_weibo.video,wen_weibo.videoHeight,wen_weibo.tehui_ids,wen_weibo.comments as commentnum');
					...
					if (!empty ($tmp)) {
						...
						$result['data']['content'] = $tmp[0]['content'];
	...
	public function topicSearch($param = '') {
		if ($this->auth->checktoken($param)) {
			if ($this->input->get('page') and $kw = mysql_real_escape_string(trim($this->input->get('kw')))) {
				...
				$this->db->select('users.alias as showname,wen_weibo.weibo_id,wen_weibo.views,wen_weibo.comments as commentnums,wen_weibo.uid,wen_weibo.uid,wen_weibo.content,wen_weibo.ctime, wen_weibo.type_data');
				$this->db->from('wen_weibo');
	...
	//	$this->db->join('wen_comment', 'wen_comment.contentid = wen_weibo.weibo_id');
		$this->db->select('wen_weibo.weibo_id,users.alias as uname,wen_weibo.views,wen_weibo.comments,wen_weibo.uid,wen_weibo.uid,wen_weibo.content,wen_weibo.ctime, wen_weibo.type_data');
	...
	public function topicList($param = '') {
		...
		$result['state'] = '000';
		if ($this->auth->checktoken($param)) {
			$sql = "SELECT w.views,w.comments,w.content, w.uid, w.favnum,w.type_data,w.weibo_id,w.ctime,u.phone,u.email,u.alias,w.commentnums ";
			...
			if ($this->input->get('uid')) {
				if ($this->input->get('type')) {
					$sql .= ' WHERE w.uid = ' . $this->input->get('uid') . " AND w.type=1 AND (w.dataType = '{$type}' OR INSTR(w.tags,'{$type}'))";
				} else {
					$sql .= ' WHERE w.type&25 AND w.uid = ' . $this->input->get('uid');
				}
			} else {
				if ($this->input->get('type')) {
					$sql .= " WHERE w.type&25 AND (w.dataType = '{$type}' OR INSTR(w.tags,'{$type}'))";
				} else {
					$sql .= " WHERE w.type&25 ";
				}
			}
		...
		if ($this->auth->checktoken($param)) {
			if ($wid = $this->input->get('weibo_id')) {
				if(!($rs = $this->alicache->get($_SERVER['REQUEST_URI']))){
					...
					$this->db->select('wen_comment.comment,wen_comment.alias,wen_comment.cTime,wen_comment.data,users.email ');
