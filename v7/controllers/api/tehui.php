...
$sql = "update partner SET {$apls} = {$apls}+1 where id = {$info[0]['partner_id']} LIMIT 1";
	...
	private function payCall($order) {
		...
		//UPDATE buy_id
		$SQL = "UPDATE `order` o,(SELECT max(buy_id)+1 AS c FROM `order` WHERE state = 'pay' and team_id = '{$order['team_id']}') AS c SET o.buy_id = c.c, o.luky_id = 100000 + floor(rand()*100000) WHERE o.id = '{$order['order_id']}' AND buy_id = 0;";
		$this->tehuiDB->query($SQL);
