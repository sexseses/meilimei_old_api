...
class user extends CI_Controller {
	...
	public function freg($param) {
		...
		if ($this->auth->checktoken($param)) {
			...
			if ($this->input->post() && $this->notlogin) {
				...
				$device_sn = $this->input->post('phone');
					...
					if ($this->form_validation->run()) {
                       ...
                       $result['username'] = $username;
						...
						if ($this->input->post('username') and $data['user_id'] and $this->_check_user_alias($this->input->post('username'), $result)) {
							$updateUser = array ();
							$result['username'] = $updateUser['alias'] = $updateUser['username'] = $this->input->post('username');
				...
				if (($phnum != '' || $email != '') ) {
					if ($this->form_validation->run()) {
						...
						if ($phnum) {
							$this->sms->sendSMS(array (
								"{$phone}"
							), "感谢你注册美丽神器 APP,你的账户:{$phone},密码:{$password},请妥善保管" . '退订回复TD ');
						}
						//send tehui
						$i = 0;
						for($i = 0; $i<5; $i++){
			...
			if ($this->input->post("username") || $this->notlogin) {
				...
				if ($this->form_validation->run()) {
					...
					if ($this->wen_auth->login($username, $password, true)) {
                        ...
                        $clinetdata['clientid'] = $this->input->post('clientid');
						//log user location
                        $this->db->where('id', $uid);
                        $this->db->update('users', $clinetdata);
	...
	function authlogin($param = '') {
		...
		$result['ustate'] = '001';
		if ($this->auth->checktoken($param)) {
			if ($this->input->post('type') and $token = $this->input->post('apitoken')) {
				if ($query = $this->Users_model->get_by_ref_id($this->input->post('type') . '_' . $this->input->post('apitoken')) AND $this->notlogin) {
					$row = $query->row();
					if ($query->num_rows() == 0) {
						$result['state'] = '400';
						echo json_encode($result);
	...
	private function _check_user_alias($username = '', & $result) {
		if (!strpos($username, 'null') and $this->str_len($username) > 3 and $this->str_len($username) < 30 and preg_match('/^[\x80-\xff_a-zA-Z0-9]+$/', $username)) {
			...
			$state2 = 0;
			if ($this->uid) {
				$state2 = $this->db->query("select count(*) as num from users where id = {$this->uid} and  alias = '{$username}' ")->result_array();
				$state2 = $state2[0]['num'];
			}
				...
				if (true || $this->session->userdata('veryCodeTime') > time()) {
					...
					if ($this->input->get('type')) {
						if (!$this->_check_phone_no($phone)) {
							$message = '[编号:' . $sn . ']您的手机验证码是：' . $code . '退订回复TD';
							$time = time() + 120;
							$result['vcode'] = $code;
