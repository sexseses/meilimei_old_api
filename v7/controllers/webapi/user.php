			...
			if (($phnum != '' || $email != '') ) {
				if ($this->form_validation->run()) {
					...
					if ($phnum) {
						$this->sms->sendSMS(array (
							"{$phone}"
						), "【美丽神器】感谢你注册美丽神器 APP,你的账户:{$phone},密码:{$password},请妥善保管" . '退订回复0000 ');
					}
					//send tehui
					$i = 0;
					for($i = 0; $i<5; $i++){
			...
			if (true ||$this->alicache->get(md5("veryCodeTime".$code)) > time()) {
				...
				if ($this->input->get('type')) {
					if (!$this->_check_phone_no($phone)) {
						$message = '【美丽神器】[编号:' . $sn . ']您的手机验证码是：' . $code . '退订回复0000';
						$time = time() + 120;
						$result['vcode'] = $code;
					...
					if ($this->_check_phone_no($phone)) {
						$message = '【美丽神器】手机验证码是：' . $code . '退订回复0000 ';
						$time = time() + 120;
						$result['vcode'] = $code;
						$this->session->set_userdata('veryCodeTime', $time);
						$this->sms->sendSMS(array (
	...
	function sendresetpwdsms($param = '') {
		...
		if ($phone = $this->input->get('phone')) {
			if ($this->session->userdata('veryCodeTime') > time() || !$this->session->userdata('veryCodeTime')) {
				...
				$message = '【美丽神器】您的手机验证码是：' . $code . '退订回复0000 ';
				$time = time() + 120;
				$this->session->set_userdata('veryCodeTime', $time);
				$this->sms->sendSMS(array (
					"{$phone}"
