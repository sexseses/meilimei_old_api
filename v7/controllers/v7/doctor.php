...
class doctor extends MY_Controller {
    ...
    public function jsearch(){
        ...
        /*if($jg){
            ...
            $SQL = "select u.alias as name, p.company as company from users as u LEFT JOIN user_profile as p ON u.id=p.user_id where u.role_id=2 and u.alias like '{$c}%' limit {$offset} ,10";
        //}
        if($c){
...
//		}*/
        ...
        if (!empty ($tmp)) {
            foreach ($tmp as $row) {
                ...
                $cid = $this->getCompanyId($row['company']);
    ...
    public function talk($param = '') {
        ...
        if ($this->notlogin) {
            ...
            if ($qid = $this->input->post('qid') and ((isset ($_FILES['attachPic']['tmp_name']) and $_FILES['attachPic']['tmp_name']!='') OR $this->input->post('comment'))) {
                ...
                $d = $this->db->insert('wen_comment', $data);
                ...
                if($tmp['0']['id']=$this->uid){
                    ...
                    if(!empty($tmp2)){
                        $result['debug']= $this->db->query("UPDATE `question_state` SET `new_reply` =`new_reply`+ 1  WHERE `qid` ={$qid} AND uid = {$auid}");
                    }else{
                        $insertData = array();
                        $insertData['uid'] =$auid;
                        $insertData['qid'] =$qid;
                        ...
                        $d = $this->common->insertData('question_state', $insertData);
                        $result['debug'] =$d;
                    }
                }
            } else {
    ...
    public function uinfo($param = '') {
        ...
        if($uid){
            ...
            $result['data']['HospitalId'] = $this->getCompanyId($result['data']['company']);
            $result['data']['fanCount'] = $this->Diary_model->getFunCount($uid);
            $result['data']['chenghao'] = '';
            ...
            $result['data']['casescore'] = rand(3,5);
    ...
    private function getqustions($uid){
        $sql = "SELECT user_profile.sex,q.title,q.cdate,a.qid,w.type_data, w.imgfile FROM wen_questions as q LEFT JOIN wen_weibo as w ON w.q_id = q.id LEFT JOIN user_profile ON q.fUid=user_profile.user_id LEFT JOIN wen_answer as a ON a.qid = q.id WHERE a.uid={$uid} and  w.type_data like '%savepath%' GROUP BY a.qid order by q.cdate desc limit 3";
        ...
        foreach($rmp as $r){
            ...
            if(isset($tmf['pic']['savepath'])){
                ...
                if(empty($r['imgfile'])) {
                    $r['pictures'][] = $this->remote->getLocalImage($tmf['pic']['savepath'], 160);
                }else{
                    $r['pictures'][] = $this->remote->getQiniuImage($r['imgfile'], 160);
                }
            }
            $r['type_data'] = null;
            switch ($r['sex']) {
    ...
    public function questions($param = '') {
        ...
        if ($uid = intval($this->input->get('uid'))) {
            ...
            $sql = "SELECT wen_questions.id,wen_questions.position as tag,wen_questions.fuid,wen_questions.title, wen_questions.cdate,user_profile.user_id,user_profile.sex,wen_questions.address,wen_weibo.type_data ";
            ...
            $sql .= "WHERE wen_answer.uid = {$uid} and wen_weibo.type=4 ORDER BY wen_questions.id DESC LIMIT $start,$limit";
            ...
            foreach ($tmp as $row) {
                ...
                $item ['basicinfo'] = $this->getBasicInfo($user[0]);
                ...
                if(isset($tmps[1]['id'])){
                    ...
                    if(empty($ptmp[0]['imgfile'])) {
                        $r['savepath'] = $this->remote->getLocalImage($ptmp[0]['savepath'], 160);
                    }else{
                        $r['savepath'] = $this->remote->getQiniuImage($ptmp[0]['imgfile'], 160);
                    }
    ...
    public function getDoctorRankinglist($param = ''){
        ...
        $result['data']['Darenlist'][] = array();
        if(!empty($rs)){

            foreach($rs as $item){

                $this->db->from('users');
                $this->db->where('item',$item['name']);
                $this->db->select('user_profile.Fname,user_profile.Lname,user_profile.company,user_profile.user_id,users.suggested,users.grade');
                $this->db->join('user_profile','users.id=user_profile.user_id');
                $this->db->limit(5);
                $this->db->order_by('user_profile.sort','desc');
                $this->db->where('user_profile.isrecommend',1);
                $doctors = $this->db->get()->result_array();
                $r = array();
                $r['title'] = $item['name'];
                if(empty($item['imgfile'])) {
                    $r['imgurl'] = $this->remote->getLocalImage($item['img_png']);
                }else{
                    $r['imgurl'] = $this->remote->getQiniuImage($item['imgfile']);
                }

                if(!empty($doctors)){
                    foreach($doctors as $doctor){
                        $doctor['username'] = $doctor['Lname'].$doctor['Fname'];
                        $doctor['thumb'] = $this->profilepic($doctor['user_id'],2);
                        unset($doctor['Lname']);
                        unset($doctor['Fname']);
                        $r['list'][] = $doctor;
                    }
