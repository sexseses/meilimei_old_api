...
class diary extends MY_Controller {
		...
		if($nid > 0 ){
			...
			if(!empty($rs)){
				...
				foreach($rs as $item){
					...
					$this->db->select('itemname,title');
					$this->db->where('ncid', $item['ncid']);
					$note_category = $this->db->get('note_category')->row_array();
					$item['title'] = $note_category['title'];
					$item['items'] = array();

					if(!empty($item['imgfile'])){
						$item['imgfile']  = $this->remote->getQiniuImage($item['imgfile']);
					}else{
						$item['imgfile'] = $this->remote->getLocalImage($item['imgurl']);
					}

					if(!empty($note_category['itemname'])) {
						$this->db->select('name');
						$this->db->where_in('id', array_filter(explode('@', $note_category['itemname'])));

						$item['items'] = $this->db->get('crm_items')->result_array();
					}else{
						$item['items'] = array();
					}
					$item['day'] = $item['ncid'];
					if (time() - $item['created_at'] < 3600 * 10) {
						if (time() - $item['created_at'] < 3600) {
							$item['created_at'] = intval((time() - $item['created_at']) / 60) . '分钟前';
						} else {
							$item['created_at'] = intval((time() - $item['created_at']) / 3600) . '小时前';
						}
					} else {
						$item['created_at'] = date('Y年m月d日', $item['created_at']);
					}

					$item['doctor'] = $this->getDoctorComments();
					$result['data'] = $item;
	...
	public function sendComment($param = ''){
		...
		if($nid && $content && $this->uid){
			...
			$result['data'] ['cid']= $this->Diary_model->saveComment($data);
			...
			$result['notice'] = '评论成功!';
			$result['pageCount'] = ceil(intval($result['total_comments'])/10);
			$result['pageSize'] = 10;
	...
	public function getDiaryListForCategory($param = ''){
		...
		if(intval($ncid) > 0){
			...
			$this->db->select('ncid, uid, jifen, title, itemname, mechanism');
			$this->db->where('ncid',$ncid);
			$note_category = $this->db->get('note_category')->row_array();

			if(!empty($note_category)){

				$temp = $this->Diary_model->get_user_by_username($note_category['uid']);

				$note_category['thumb'] = $this->remote->thumb($note_category['uid'], 105);
				$username = !empty($temp[0]['alias']) ? $temp[0]['alias'] : $temp[0]['username'];
				$note_category['username'] = $username;
				if (preg_match("/^1[0-9]{10}$/", $note_category['username'])) {

					$note_category['username'] = $this->getAlias($note_category['username']);
				}
				$note_category['jifen'] = !empty($note_category['jifen'])?$note_category['jifen']:0;
				$note_category['level'] = $this->getLevel($temp[0]['jifen']);
				$note_category['age'] = $this->getAge($note_category['uid']);
				$note_category['sex'] = $temp[0]['sex']; //1 nv  2 nan
				if(!empty($note_category)) {

					if(!empty($note_category['itemname'])){
						$this->db->select('name');
						$this->db->where_in('id', array_filter(explode('@', $note_category['itemname'])));
						$note_category['items'] = $this->db->get('crm_items')->result_array();
					}else{
						$note_category['items'] = array();
					}
				}else{
					$note_category['items'] = array();
				}

				$this->db->where('user_id', $note_category['uid']);
				$userprofile = $this->db->get('user_profile')->row_array();

				$note_category['city'] = $userprofile['city'];

				$this->db->where('userid', $note_category['mechanism']);
				$company = $this->db->get('company')->row_array();
				if(!empty($company)){
					$note_category['hospital'] = array('name'=>$company['name'], 'grade'=>$company['grade'], 'verify'=>1, 'is_recommend'=>1, 'sign'=>'医院很不错');
				}else{
					$note_category['hospital'] = array();
				}

				$this->db->select('created_at,imgfile,total_comments,content');
				$this->db->where('ncid', $ncid);
				$this->db->group_by('created_at');
				$this->db->order_by('created_at', 'desc');
				$notes = $this->db->get('note')->result_array();
				$note_category['notes'] = array();
				if(!empty($notes)){

					foreach($notes as $note){

						$note_category['notes'][] = $note;
					}
				}else{
					$note_category['notes'] = array();
				}

				$note_category['tehui'] = array(array('id'=>'id', 'title'=>'韩式全切双眼皮', 'delivery'=>'delivery','image'=>'http://img1.imgtn.bdimg.com/it/u=2200543776,3270904349&fm=21&gp=0.jpg','mechanism'=>'上海百达丽医院', 'team_price'=>'1.99', 'market_price'=>'2.00', 'now_number'=>20));

			}
			$result['data'] = $note_category;
		}else{
			$result['state'] = 1016;
		}
		echo json_encode($result);
	...
	public function getDiaryByCategoryId(){
		...
		$page =  !empty($page) ?$this->input->get('page'):1;
		$offset = (intval($page)-1)*10;
		
		if(intval($ncid) > 0){
			$this->db->select('ncid, itemname');
			$this->db->where('ncid',$ncid);
			$note_category = $this->db->get('note_category')->row_array();

			$this->db->select('created_at,imgfile,total_comments,content,nid');
			$this->db->where('ncid', $ncid);
			$this->db->group_by('created_at');
			$this->db->order_by('created_at', 'desc');
			...
			if(!empty($notes)){
				...
				foreach($notes as $note){
					if ($note['created_at'] > time() - 3600) {
						$note['created_at'] = intval((time() - $note['created_at']) / 60) . '分钟前';
					} else {
						$note['created_at'] = date('Y年m月d日', $note['created_at']);
					}
					$note['day'] = $ncid;
					if(!empty($note)) {
						...
						if(!empty($note_category['itemname'])){
							$this->db->select('name');
							$this->db->where_in('id', array_filter(explode('@', $note_category['itemname'])));
							$note['items'] = $this->db->get('crm_items')->result_array();
						}else{
							$note['items'] = array();
						}
					}else{
					...
					$note['imgfile'] = $this->remote->getQiniuImage($item['imgfile']);
					$note['doctor'] = $this->getDoctorComments();
					$result['data'] [$ncid][]= $note;
				}
				...
				foreach($result['data'] as $key=>$item){
					$tmp = array();
					$tmp['created_at'] = $item[0]['created_at'];
					$tmp['day'] = $item[0]['day'];
					$tmp['all'] = $item;
					$temp[] = $tmp;	
				}
	...
	public function getCommentList($param = '')
	{
		...
		if (intval($nid) > 0) {
			...
			$tmp = $this->Diary_model->getCommentList($nid, $lastid, 10, $sort);
			...
			if (!empty($tmp)) {
				...
				foreach ($tmp as $item) {
					...
					if (preg_match("/^1[0-9]{10}$/", $item['username'])) {
						...
						$item['username'] = $this->getAlias($item['username']);
					}
					...
					if (!empty($item['reply'])) {
						...
						foreach ($item['reply'] as $it) {
							...
							if (preg_match("/^1[0-9]{10}$/", $it['fromusername'])) {
								$it['fromusername'] = $this->getAlias($it['fromusername']);
							}
							if (preg_match("/^1[0-9]{10}$/", $it['tousername'])) {
								$it['tousername'] = $this->getAlias($it['tousername']);
							}
							...
							$it['thumb'] = $this->remote->thumb($it['fromuid'], 105);;
							$reply[] = $it;
						}
	 ...
	 * @param string $param
	 */
	public function sendDiaryCategory($param = ''){
		...
		if($this->uid){
			...
			$data = array('uid' => $this->uid,'title' => $title, 'itemname' => $items,'operation_time'=>strtotime($operation_time),'created_at' => time(), 'mechanism'=>$mid ,'updated_at' => time());
			$result['debug'] = $data;
			$isCategory = $this->Diary_model->isCategory($title, $this->uid);
			if (intval($isCategory) <= 0) {
				$this->Diary_model->addNoteCategory($data);

			} else {
				$result['state'] = '1022';
			}

			$ncid = $this->Diary_model->getLastID();

			$result['data']['ncid'] = $ncid;

			$array_filter = array();
			$array_filter = array_filter(explode('@',$tids));

			if(!empty($array_filter) && intval($ncid) > 0){

				foreach($array_filter as $tid){

					$this->db->insert('note_category_tehui', array('ncid'=>$ncid, 'tehuiid'=>$tid, 'is_display'=>1, 'create_time'=>time()));
				}
			}
		}else{
			$result['state'] = '1004';
	...
	public function getDiaryCategory($param = ''){
		...
		if($this->uid > 0){
			...
			$this->db->select('title,created_at,ncid,itemname');
			$rs = $this->db->get('note_category')->result_array();
			...
			if(!empty($rs)){
				...
				foreach($rs as $item){
					...
					$item['created_at'] = date('Y年m月d日', $item['created_at']);
					$arr_item = array_filter(explode('@', $item['itemname']));
					if(!empty($arr_item)) {
						$this->db->select('name');
						$this->db->where_in('id', array_filter(explode('@', $item['itemname'])));
	...
	public function sendDiary($param = ''){
		...
		if ($content && $this->uid) {
			...
			$data = array('uid' => $this->uid, 'ncid'=>$ncid, 'content' => $content, 'imgfile'=> $imgfile,'created_at' => strtotime($date), 'updated_at' => time());

			$this->Diary_model->saveUserDiary($data);

			$result['data']['nid'] = $this->Diary_model->getLastID();;
		} else {
			$result['state'] = '1004';
		}
	...
	public function getIndexDiary(){

		$result['state'] = '1000';
		$result['data'] = array();

		$this->db->select('ncid,uid,title,itemname,jifen');
		$this->db->where('is',1);
		$this->db->limit(3);
		$note_category = $this->db->get('note_category')->result_array();

		if(!empty($note_category)){

			foreach($note_category as $item){

				$this->db->where('ncid',$item['ncid']);
				$item['num'] = $this->db->get('note')->num_rows();
				$item['thumb'] = $this->profilepic($item['uid']);

				$this->db->select('content,imgfile,imgurl');
				$this->db->where('ncid', $item['ncid']);
				$this->db->limit(1);
				...
				$item['NmrjNmeidou'] = impolde(',',$item['items']) .'/'. ;
				$item['sign'] = $note['content'];
				if(!empty($note['imgfile'])){
					$item['imgfile'] = $this->remote->getQiniuImage($note['imgfile']);
				}else{
					$item['imgfile'] = $this->remote->getLocalImage($note['imgurl']);
				}
				$result['data'][] = $item;
			}
		} else {
	...
	public function getIndexDiaryCategory(){
		$result['state'] = '1000';
		$result['data'] = array();

		$this->db->select('nc.ncid,nc.uid,nc.title,nc.itemname,nc.jifen, u.alias, u.jifen as score');
		$this->db->from('note_category nc');
		$this->db->join('users u', 'nc.uid=u.id');
		$this->db->where('nc.is',1);
		$this->db->limit(10);
		$note_category = $this->db->get()->result_array();
		...
		if(!empty($note_category)){
			...
			foreach($note_category as $item){
				...
				$this->db->select('sex, city');
				$this->db->where('user_id', $item['uid']);
				$user_profile = $this->db->get('user_profile')->row_array();
				$item['sex'] = $user_profile['sex'];
				...
				$this->db->select('nid,content,imgfile, imgurl');
				$this->db->where('ncid', $item['ncid']);
				$this->db->order_by('nid', 'desc');
				$note = $this->db->get('note')->result_array();
				$item['note'] = array();

				if(!empty($note)){
					$item['sign'] = $note[0]['content'];
					$num = 0;
					foreach($note as $item_note){
						$item_note['day'] = $item_note['nid'];
						if(!empty($item_note['imgfile'])){
							$item_note['imgfile'] = $this->remote->getQiniuImage($item_note['imgfile']);
						}else{
							$item_note['imgfile'] = $this->remote->getLocalImage($item_note['imgurl']);
						}
						$this->db->where('nid', $item_note['nid']);
						$note_num = $this->db->get('note_comment')->num_rows();
						$num = $num + intval($note_num);
						unset($item_note['content']);
						$item['note'][] = $item_note;
					}
					$item['comments'] =  $num;
				}else{
	...
	public function getMyTehuiByUserId(){
		...
		if($this->uid){
			...
			$this->tehuiDB->select('team.id, team.title, team.image,team.mechanism, order.id as order_id, order.money as order_price');
			$this->tehuiDB->from('order');
			$this->tehuiDB->join('team', 'order.team_id=team.id');
			$this->tehuiDB->where('order.user_id', $this->uid);
			$this->tehuiDB->group_by('order.team_id');
			$rs_tehui = $this->tehuiDB->get()->result_array();

			if(!empty($rs_tehui)) {
				foreach($rs_tehui as $rs) {


					if(empty($rs['mechanism']) || is_null($rs['mechanism'])) {
						$tehui_sql = "select * from tehui_relation where tehui_id = ?";
						$tehui_rs = $this->db->query($tehui_sql, array($rs['id']))->row_array();

						$m_id = $tehui_rs['mechanism'];
						$this->db->where('id', $m_id);
						$rs['mechanism'] = "";
						$company = $this->db->get('company')->row_array();
						$rs['mid'] = $company['user_id'];
                        //$r['debug11'] = $company;
						if ($company) {
							$rs['mechanism'] = $company;
							if($rs['mechanism']['name'] == "美丽神器APP"){
								$rs['mechanism'] = '';
							}else{
								$rs['mechanism'] = $rs['mechanism']['name'];
							}
						}
					}else{
					...
					if(!is_null($rs['mechanism'])) {


						if(strpos($rs['image'], 'http://pic.') === false && strpos($rs['image'], 'clouddn.com') === false) {
							$rs['image'] && $rs['image'] = 'http://tehui.meilimei.com/static/' . $rs['image'];
						}
	...
	public function getJigouByTehuiId(){

		$result = array();
		$result['state'] = '1000';
		$result['data'] = array();

		$company_id = intval($this->input->get('company_id'))?intval($this->input->get('company_id')):0;
