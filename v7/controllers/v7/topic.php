...
class topic extends MY_Controller {
    ...
    function getGroups(){
        ...
        $this->db->select("gid, gname, gname as gcolor,gname as gdesc,imgurl");

        $groups = $this->db->get("new_group")->result_array();

        if (!empty($groups)) {

            foreach ($groups as $key => $group) {
                $group['gcolor'] = '255,255,255';
                $result['data'][] = $group;
    ...
    function getGroup(){
        ...
        $groupid = $this->input->get('groupid');  //圈子ID

        if(intval($groupid) > 0){

            $this->db->select("g.gid, g.gname, g.topimgurl as imgurl, t.tagid, t.tname, t.type ,t.tvalue, t.imgurl as timgurl");
            $this->db->from("new_group g");
            $this->db->join("new_tags t", "g.gid = t.groupid");
            ...
            if(!empty($groups)){
                ...
                foreach($groups as $key=>$group){
                    ...
                    $result['data']['gname'] = $group['gname'];
                    $result['data']['imgurl'] = $group['imgurl'];

                    $tags = array();

                    $tags['tagid'] = $group['tname'];
                    $tags['type']  = $group['type'];
                    $tags['tvalue'] = $group['tvalue'];
                    $tags['timgurl'] = $group['timgurl'];
	...
	function getView($param = '') {
        ...
        $weibo_id = intval($this->input->get('weibo_id'));
        ...
        $this->db->select('wen_weibo.weibo_id , user_profile.sex, users.alias as uname, users.jifen, wen_weibo.uid, wen_weibo.content , wen_weibo.comment,wen_weibo.ctime');
        ...
        if(!empty($tmp)) {
            ...
            foreach ($tmp as $r) {
                ...
                $r['doctor'] = $this->getDoctorComments();
                $result['data'] = $r;
            }
        }else{
            $result['state'] = "1016";
	...
	public function addTopicWithPics($param = '') {
		...
		if ($this->uid) {
            ...
            if(intval($this->input->post('groupid')) <= 0){
                $result['state'] = '1015';
                echo json_encode($result);
                exit();
            }
			...
			if (strlen(strip_tags(trim($this->input->post('content')))) > 2) {
				$datas = array ();
                $datas['groupid'] = $this->input->post('groupid');
				$datas['uid'] = $this->uid;
				$datas['ctime'] = time();
				...
				$result['data']['weibo_id'] = $this->db->insert_id();
				...
				if(intval($this->input->post('event_id')) > 0){
					...
					$this->eventDB->insert('event_topic_detail', array('event_id'=>intval($this->input->post('event_id')), 'topic_id'=>$result['data']['weibo_id'], 'user_id'=>$this->uid, 'top'=>0, 'display'=>1, 'smscontext'=> '', 'sms'=> 'N', 'createtime'=>time()));
				}
                ///////////////////////////////////////////////////////////////////////////////
                ///上传多图功能
				...
				if(!empty($images)){
					...
					foreach($arr_images as $image){
                        if(empty($image))
                            continue;
						$p = $this->addTopicPicAC($result['data']['weibo_id'],$image);
					}
...
/*
				...
				if ($data['type'] == 'topic') {
					$updata = array ();
					$updata['newtime'] = time();
					$this->db->query("UPDATE  `wen_weibo` SET  `newtime` =  '".time()."',`commentnums` =  `commentnums`+1,comments=comments+1 WHERE `weibo_id` = {$result['data']}");
    ...
    //
    //********************************************************************//
	public function getTopicByGroupid() {
		...
		$this->db->join('users', 'users.id = wen_weibo.uid');
        $this->db->join('user_profile','users.id=user_profile.user_id');
		$this->db->select('wen_weibo.weibo_id , user_profile.sex, user_profile.city, wen_weibo.tags,users.alias as uname, users.jifen, wen_weibo.uid, wen_weibo.content , wen_weibo.commentnums as comment ,wen_weibo.ctime');
    ...
    public function getTopicByRecommend() {
        $pageSize=1;
        $page = $this->input->get('page')?$this->input->get('page'):1;
        $offset = ($page - 1) * 10;
        ...
        $this->db->join('users', 'users.id = wen_weibo.uid');
        $this->db->join('user_profile','users.id=user_profile.user_id');
        $this->db->select('wen_weibo.weibo_id , user_profile.sex, user_profile.city, users.alias as uname, users.jifen, wen_weibo.uid, wen_weibo.content , wen_weibo.commentnums as comment, wen_weibo.ctime');
    ...
    //****
    // 美圈首页推荐
    public function getGroupByTopic() {
        ...
        $this->db->join('users', 'users.id = wen_weibo.uid');
        $this->db->join('user_profile','users.id=user_profile.user_id');
        $this->db->select('wen_weibo.weibo_id , user_profile.sex, user_profile.city,users.alias as uname, users.jifen, wen_weibo.uid, wen_weibo.content , wen_weibo.tags, wen_weibo.commentnums as comment, wen_weibo.ctime');
        ...
        if(!empty($tmp)) {
            ...
            foreach ($tmp as $r) {
                ...
                if(empty($r['tags'])){
                    ...
                    $r['tags'] = array_filter(explode("@", $r['tags']));
                }
                $r['thumb'] = $this->profilepic($r['uid'], 2);
