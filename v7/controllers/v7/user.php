...
class user extends MY_Controller {
    ...
    function a1($param=''){
        ...
        echo $this->memc->G('9b75feaca779707bcedb737e54cc5e29');
        //echo $this->memc->G('472e26a204b3c1cc14adabbb1eed63d9');
        exit();
		list($v,$t) = explode(":",apc_fetch("18602162332"));
		echo $v."==".$t;
                   ...
                   $result['username'] = $username;
                    ...
                    if ($phnum) {
                        $this->sms->sendSMS(array (
                            "{$phnum}"
                        ), "账号注册成功，用户名:{$phnum},密码:{$password}。登录（美丽神器）APP看美粉的变美日记，抢购超值特价服务。打开客户端可重新设置密码。退订回复0000");
                    }
                    ...
                    $this->db->where('id',$this->wen_auth->get_user_id);
                    $rtmp = $this->db->get('users')->row_array();
                    ...
                    $this->db->query("update users set username=concat('美粉',id), alias=concat('美粉',id) where alias like '%meilizhensuo%'")->result_array();
					return $this->authlogin($this->input->post('tokentype'),$this->input->post('token'));
			} else {
				$result['state'] = '1007'; //获取参数不完整
			}
		} else {
			$result['state'] = '1007'; //获取参数不完整
		}
    ...
    //web 一键登录
    public function oneLogin($param) {
        ...
        if ($this->input->post()) {
            ...
            $phnum = $this->input->post('phone');
            $is = $this->input->post('is')?$this->input->post('is'):0;
            $token = '';
            if ($this->input->post('token') == '') {
                if ($code = $this->input->post('code')) {
                    list($scode, $t) = explode(":", apc_fetch($phnum));
            ...
            if (($phnum != '' || $email != '') ) {
                if (1) {
                    ...
                    $password = $this->input->post('password')?$this->input->post('password'):rand(100000,999999);
                    $confirmpassword = $this->input->post('confirmpassword');
                    $username == '' && $username = $phnum;
                    $username == '' && $username = $email;
                    $utype = 1;
                    ...
                    if($is) {
                        if($this->wen_auth->login($username, $password, TRUE, False)){
                            $result['notice'] = '注册成功';
                            $result['notice'] = '注册成功';
                            $result['ustate'] = '000';
                            $result['data']['state'] = '000';
                            $result['data']['uid'] = $this->wen_auth->get_user_id();
                            $result['data']['ustate'] = '000';


                            if(!$this->input->post('password') && empty($tmp[0]['phone'])){
                                $this->sms->sendSMS(array (
                                    "{$phnum}"
                                ), "账号注册成功，用户名：{$username}，密码:{$password}。登录（美丽神器）APP看美娘的日记，抢购意想不到的特惠商品，让你越来越美丽。登录客户端可以重新设置密码。" . '退订回复0000 ');
                            }
                            echo json_encode($result);
                            exit();
                        }
	...
	public function reg($param='') {
		...
		if($this->input->post('type') == 2){
			...
			if($isReg){
				...
				return $this->authlogin($this->input->post('tokentype'),$this->input->post('token'), 0);
			}
		...
		if ($this->input->post()) {
			...
			if ($phnum != '') {
				if ($this->form_validation->run()) {
                    ...
                    if (strlen($this->input->get('nickname')) >= 30)
                        $result['state'] = '1009'; //用户昵称太长，请输入10个汉字以内的文字！
                        echo json_encode($result);
                        exit();
                    }
        ...
        $jifen = $this->Score_model->addScore(43,$this->wen_auth->get_user_id());//登陆加分20
        $result['score'] = $jifen;
		echo json_encode($result);
    ...
    public function setUpdatejifen($phone){

        if(empty($phone))
            return ;
        $this->db->where('phone', $phone);
        return $this->db->update('users',array('jifen'=>1000));
    }
    public function signin($param='') {
        ...
        if ($this->input->post("username")) {
            ...
            if ($this->wen_auth->login($username, $password, true)) {
                ...
                $result['data']['type'] = $this->wen_auth->get_role_id();
                $result['data']['uid'] = $uid;
                $result['data']['phone'] = isset($tmp[0]['phone']) ? $tmp[0]['phone'] : '';
                $result['data']['expire'] = time() + 86400 * 7;
                $result['data']['token'] = md5($this->appkey . str_replace(' ', '', microtime()));
                $result['data']['version'] = FACES_VERSION;
                $result['data']['thumb'] = $this->profilepic($uid, 2);
                $result['data']['items'] = array();
                $result['data']['uploadImageToken'] = $this->qiniu->getToken();

                if (count($result['data']['items']) < 3) {
                    unset($result['data']['items']);
                    $this->addFollowUser($this->wen_auth->get_user_id());
                    $result['data']['items'] = array();
                }

                $result['data']['messageNotReadCount'] = $this->getMessagesNotReadCount($this->wen_auth->get_user_id());
                $result['data']['follows'] = $this->getFollow($uid);
                $result['data']['jifen'] = $tmp[0]['jifen'];
                $result['data']['is_question'] = 0;
                $result['data']['category_version'] = '1.0';
                $updata_session = array();
                $this->ckCoupon($result['data']['phone'], $result['data']['uid']);

                $this->db->where('uid', $uid);
                $this->db->where('created_at >=', strtotime(date("Y-m-d")));
                $this->db->where('created_at <=', strtotime(date('Y-m-d', strtotime('+1 day'))));
                $score_tmp = $this->db->get('score_logger')->row_array();
                if (empty($score_tmp)) {
                    $jifen = $this->Score_model->addScore(43, $this->wen_auth->get_user_id());//登陆加分20
                }
                $this->db->where('id', $uid);
                ...
                $result['data']['score'] = empty($jifen) ? 0 : $jifen;
                $updata_session['expire'] = $result['data']['expire'];
                $updata_session['sessionid'] = $result['data']['token'];
                $this->db->where('id', $uid);
                $this->db->update('users', $updata_session);

                $this->memc->S($result['data']['token'], $this->wen_auth->get_user_id());
                $result['data']['xxx'] = $result['data']['token'];
                if ($clientid = $this->input->post('clientid')) {
                    $this->setUpdateClient($uid, $clientid);
                }
                unset($result['data']['expire']);
                //log user location
	...
	private function authlogin($type, $token, $isReg=1) {
		$result['state'] = '000';
		...
		if ($token) {
			...
			if ($query) {
				...
				if ($query->num_rows() == 0) {
					$result['state'] = '1006'; //第三方用户不存在
					echo json_encode($result);
					exit;
				}
				if ($row->banned > 0) {
					$result['state'] = '1005'; //该用户被禁止登陆
				} else {
					...
					$result['data']['version'] = FACES_VERSION;
					$result['data']['username'] = $this->wen_auth->get_username();
					$result['data']['type'] = $this->wen_auth->get_role_id();
					$result['data']['uid'] = $this->wen_auth->get_user_id();
                    $result['data']['isReg'] = $isReg;
                    $result['data']['phone'] = isset($row->phone)?$row->phone:'';
                    $result['data']['uploadImageToken'] = $this->qiniu->getToken();
                    $update_session = array();

                    $result['data']['expire'] = time()+86400*7;
					$result['data']['token'] = md5($this->appkey.str_replace(' ','',microtime()));
					$this->db->query("update users set sessionid='".$result['data']['token'] ."',expire='".$result['data']['expire']."' where id=".$this->wen_auth->get_user_id()." limit 1");
                    $result['data']['items'] = $this->getUserItems($this->wen_auth->get_user_id());
                    $this->memc->S($result['data']['token'], $this->wen_auth->get_user_id());
                    if(count($result['data']['items']) < 3){
                        unset($result['data']['items']);
                        $this->addFollowUser($this->wen_auth->get_user_id());
                    ...
                    $score_tmp = $this->db->get('score_logger')->row_array();
                    if(empty($score_tmp)){
                        $jifen = $this->Score_model->addScore(39,$row->id);//登陆加分20
                        $result['data']['score'] = empty($jifen)?0:$jifen;
                    }else{
                        $result['data']['score'] = 0;
                    }
                    $result['data']['is_question'] = 0;
                    $result['data']['jifen'] = !empty($row->jifen)?$row->jifen:0;
                    $result['data']['jifen'] = $result['score'];
                    $result['data']['category_version'] = '1.0';
                    $result['data']['follows'] = $this->getFollow($this->wen_auth->get_user_id());
					$result['data']['thumb'] = $row->icon?$row->icon:$this->profilepic($result['uid'], 2);
                    $result['data']['uid'] = $this->wen_auth->get_user_id();
                    unset($result['data']['expire']);
                    //$result['city'] = isset($row->city)?$row->city:'';
                    if(isset($row->age)) {
                        $result['age'] = $this->getAge($this->wen_auth->get_user_id());
                    }else{
                        $result['age'] = '';
                    }
...
//testing
    ...
    private function addFollowUser($uid){
        ...
        return 1;//$this->db->insert('user_fav', $user_fav);
    }
	...
	function getinfo($param='') {
        ...
        if($uid) {
            ...
            $result['data']['age'] = $this->getAge(intval($result['data']['id']));
            $result['data']['uname'] = '';
            ...
            $result['data']['itemsCount'] = count($this->getFollowItem($uid)) ? count($this->getFollowItem($uid)) : 0;
            $followItem = $this->getFollowItem($uid);
            $str = '';
            if(!empty($followItem)){
                foreach($followItem as $it){
                    if($str == ''){
                        $str = $it['name'];
                    }else{
                        $str .= ','.$it['name'];
    ...
    private function getFollowItem($uid){

        if(intval($uid) <= 0)
            return;
        $this->db->select('n.name');
        $this->db->where('w.fid',$uid);
        $this->db->where('w.type',9);
        $this->db->from('wen_follow w');
        $this->db->join('new_items n','w.uid=n.id');
    ...
    private function ckCoupon($mobile,$userid){
        ...
        if(!empty($mobile)) {
            $csql = "select * from coupons_sn where mobile = $mobile and states = 'N' and isschool = 0";
            ...
            $csql = "select * from coupons_sn where mobile = $mobile and states = 'N' and isschool = 1";
            $crs = $this->eventDB->query($csql)->result_array();
    ...
    public function getSettingInfo($params = ''){
        ...
        if($this->uid){
            ...
            $orderCount = $this->tehuiDB->query("select *from `order` where state = 'unpay' and user_id={$this->uid} and create_time > ".$time)->num_rows();
            $messageNotReadCount =  $zixunNotReadCount + $zanNotReadTotal + $commnetTotal + $orderCount;
	...
	*/
		...
		if ($this->uid) {
			...
			if ($username) {
				...
				if ($query[0]['id'] == $this->uid || count($query) == 0) {
                    ...
                    if(strlen($data['alias']) >= 30){
                        $result['notice'] = '用户昵称太长，请输入10个汉字以内的文字！';
                        $result['state'] = '017';
                        echo json_encode($result);
                        exit();
                    }
                    ...
                    if($this->input->post('age')){
                        ...
                        $s = $this->db->query("update users set age='{$datainfo[age]}' where id='{$uid}'");
                    }
                    $result['debug'] = $s;
                    $this->db->where('id',$this->uid);
                    $rs = $this->db->get('users')->result_array();
                    if(intval($rs[0]['age']) <= 0 && intval($this->input->post('age')) >0 ){
                        $this->db->where('id',$uid);
                        $this->db->update('users',array('jifen'=>$rs[0]['jifen'] + 600));
                        $result['score'] = 600;
                    }else{
                        $result['score'] = 0;
                    }
    ...
    function updateNickname(){
        ...
        if ($this->uid) {
            $this->db->where('id', $this->uid);
            $this->db->update('users', array('alias'=>$this->input->get('nickname')));
        } else {
            $result['notice'] = '账户未登入！';
            $result['ustate'] = '012';
        }
    ...
    function updateuicon($param = '') {
        ...
        if ($this->uid) {
            if (isset ($_FILES['attachPic']['tmp_name']) && $_FILES['attachPic']['tmp_name']) {
                $result['debug'] = $_FILES;
                $f = $this->thumb($this->uid, $_FILES['attachPic']['tmp_name']);
                $datainfo = array();
                $datainfo['icon'] = 0;
                $this->db->where('id',$this->uid);
                $this->db->update('users',$datainfo);
                $result['notice'] = '更新成功！';
                $result['file'] = $this->remote->thumb($this->uid);
            }
        } else {
            $result['notice'] = '账户未登入！';
            $result['ustate'] = '012';
    ...
    function updateUserInfo($param = '') {
        ...
        if ($this->uid) {
            ...
            $rsp = $this->db->get('user_profile')->result_array();
    ...
    function websendsms($param = '') {
        $result['state'] = '000';
        if ($phone = $this->input->get('phone')) {
            $sn = rand(10000, 99999);
            $result['sn'] = $sn;
            $result['notice'] = '已发送';
            $codeExpire = $this->input->get('code');
