...
class comments extends MY_Controller
{
    ...
    public function sendComment($param = '')
    {
        ...
        if (!$this->uid) {
            ...
            if (($type = strip_tags($this->input->post('type'))) and ($contentid = intval($this->input->post('weibo_id')))) {
                $pid = intval($this->input->post('pid'));
                $touid = intval($this->input->post('touid'));
               if (strlen($this->input->post('content')) < 2) {
                    $result['state'] = '1013';
                    echo json_encode($result);
                    exit;
                }
                ...
                $Idata['touid'] = $touid;
                $Idata['cTime'] = time();
                $Idata['comment'] = strip_tags($this->input->post('content'));

                $this->db->insert('wen_comment', $Idata);
                $result['data']['cid'] = $this->db->insert_id();
    ...
    public function getCommentList($param = '')
    {
        ...
        $result['data'] = array();
        $pageSize = 10;
        if (($type = trim($this->input->get('type'))) and ($contentid = intval($this->input->get('weibo_id')))) {
                $this->db->where('contentid', $contentid);
                $topicTotal = $this->db->get('wen_comment')->num_rows();
                $lastid = $this->input->get('lastid') ? $this->input->get('lastid') : 0;
                $sort = $this->input->get('sort')?$this->input->get('sort'):'ASC';
                $fields = 'users.alias as uname,users.phone,users.id as uid, users.jifen, c.id,c.contentid,c.touid,c.comment,c.cTime,c.pid';
                ...
                if ($sort == "ASC") {
                    ...
                    if ($lastid) {
                        ...
                        $sql .= "WHERE type = '{$type}' and c.id > {$lastid} and contentid={$contentid} and is_delete=0  and pid=0 and users.banned=0 order by c.id ASC limit $pageSize";
                    } else {
                        $sql .= "WHERE type = '{$type}' and contentid={$contentid} and is_delete=0  and pid=0 and users.banned=0 order by c.id ASC limit 0, $pageSize";
                    }
                } else {
                    //效率有问题
                    ...
                    if ($lastid) {
                        ...
                        $sql .= "WHERE type = '{$type}' and c.id < {$lastid} and contentid={$contentid} and is_delete=0  and pid=0 and users.banned=0 order by c.id DESC limit $pageSize) as tmp order by id ASC";
                    } else {

                        $sql = "SELECT {$fields} FROM wen_comment as c LEFT JOIN users ON users.id = c.fuid ";
                        $offset = intval($topicTotal) - intval($pageSize);
                        if($offset < 0)
                            $offset = 0;
                        $sql .= "WHERE type = '{$type}' and contentid={$contentid} and is_delete=0  and pid=0 and users.banned=0 order by c.id ASC limit $offset, $pageSize";
                    }
                }
                $tmp = $this->db->query($sql)->result_array();
                ...
                foreach ($tmp as $r) {
                    ...
                    $r['level'] = $this->getLevel(intval($r['jifen']));
                    //clear user new_reply
                    ...
                    $sql = "SELECT {$fields} FROM wen_comment as c LEFT JOIN users ON users.id = c.fuid ";
                    $sql .= "WHERE type = '{$type}' and c.pid={$r['id']} and users.banned=0 and is_delete=0 order by c.id ASC";
                    $tmps = $this->db->query($sql)->result_array();
                    if (!empty($tmps)) {
                        foreach ($tmps as $item) {
                            if (time() - $r['cTime'] < 3600 * 10) {
