...
class items extends MY_Controller {
    ...
    function getItems($param = '') {
        ...
        if($pid > 0) {
            ...
            $tmp = $this->db->get('crm_items')->result_array();
            $result['data'] = array();
            foreach ($tmp as $r) {
                $this->db->select('id, name');
                $this->db->where('pid',$r['id']);
                $res = $this->db->get('crm_items')->result_array();
                if(!empty($res)){
                    foreach($res as $rs){
                        $r['child'][] = $rs;
                    }
    ...
    function getIndexItems($param = '') {
        ...
        if($pid > 0) {
            $this->db->select('id, name, img_s as imgurl');
            $this->db->where('pid',$pid);
            $this->db->order_by("id", "desc");
            $tmp = $this->db->get('crm_items')->result_array();
            $result['data'] = array();
