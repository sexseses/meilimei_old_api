...
// 	    $this->uid = $uid;
    ...
    public function getTehuiByJigouId(){
        ...
        if(!empty($m_id)){
            ...
            $tehui_fields = 't.id,t.title,t.image,t.team_price, t.now_number,t.market_price,t.delivery,t.deposit';
            $time = time();
            $tehui_condition = " and (t.team_type='normal' and t.begin_time <= '{$time}' and t.end_time >= '{$time}' and flashSale = 0 and ((mechanism='" . $m_id. "') $sql))";
...
// 	        }
    ...
    public function getProductList($param = '') {
        ...
        if ($page) {
            $time = time();
            $start = ($page -1) * 10;
            $fields = 't.id,t.title,t.image,t.team_price, t.now_number,t.market_price,t.delivery, t.mechanism,t.is_top, t.top_sort';
            $condition = "t.team_type='normal' and t.begin_time <= '{$time}' and t.end_time >= '{$time}' and flashSale = 0 and display=1";
                ...
                if (($team['id'] && $team['id'] == $id) && (!empty ($quantity) && $p_store > $team['p_warnning'])) {
                    if ($this->tehuiDB->insert('order', $data)) {
                        ...
                        if ($data['jifen']) {
                            ...
                            //$this->Score_model->addLogger($this->uid, "订单抵扣", $data['jifen']);
                        }
                        if ($data['origin'] == 0) {
                            $this->pay($result['pay_id']);
                            $result['notice'] = '已付款成功！';
