        ...
        if(count($rs)<=0){
            ...
            if (!empty ($tmp)) {
                ...
                if ($this->result['data']['image'] != '') {
                    if(strpos($this->result['data']['image'], 'http://pic.') !== 0) {
                ...
                if ($this->result['data']['image1'] != '') {
                    if(strpos($this->result['data']['image1'], 'http://pic.') !== 0) {
                ...
                if ($this->result['data']['image2'] != '') {
                    if(strpos($this->result['data']['image2'], 'http://pic.') !== 0) {
...
//echo $this->tehuiDB->last_query();die;
	...
	public function getUserOrderList() {
	    ...
	    if ($this->uid) { 
	        ...
	        $this->tehuiDB->select('order.pay_id,order.id,order.express,order.express_no,order.comment_time,order.quantity, order.state,team.team_price, team.image,team.title,team.summary,order.create_time,team.delivery');
...
// 	    $this->uid = $uid;
	...
	public function getTehuiByJigouId(){
	    ...
	    if(!empty($m_id)){
	        ...
	        $tr_rs = $this->db->query($tr_sql,array($m_id))->result_array();
	    }else{
	        $result['ustate'] = '001';
	        $result['notice'] = '参数不全！';
	        $result['state'] = '012';
	    ...
	    if($tr_rs){
	        ...
	        $tehui_condition = "and t.id in ({$tr_rs})";
...
// 	            }
	...
	private function gdetail($content,$title){
	    $content = preg_replace(array('/ style=\".*?\"/'),array(''),$content);

        return '<div id="content" class="mainc">'.str_replace(array('&nbsp;','<o:p></o:p>'),array('',''),$content).'</div> ';
	}
	...
	public function order($param = '') {
		...
		if (($quantity = $this->input->post('quantity')) and $id = intval($this->input->post('id'))) {
			...
			$data['fare'] = 0;//$data['express'] == 'Y' ? $express_price : 0;
		   ...
		   if ((stristr($head, 'iPhone') and !stristr($head, 'U;')) OR stristr($head, 'ipod')) {
			...
			if ($data['card_sn']) {
			    ...
			    if (empty ($coupon_card)) {
			        ...
			        $data['credit'] = $cards['credit'];
			        $data['origin'] -= $cards['credit'];
			        $update['consume'] = 'Y';
			        $this->db->where('sn', $data['card_sn']);
			...
			if (intval($team['p_store']) > intval($team['p_warnning'])) {
	...
	public function payInfo($param) {
		...
		if ($this->uid) {
		    $id = $this->input->get('id');
			if ($id) {
				$this->tehuiDB->select('team.delivery,order.origin,category.name as express_name,order.price,order.pay_id,order.pay_id,order.jifen,order.mobile,order.fare,order.credit,order.state,order.quantity,team.title');
				$this->tehuiDB->where('order.id', $id);
