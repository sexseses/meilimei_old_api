...
class user extends MY_Controller {
	...
	*  $phone int 
    ...
    public function setUpdateJifen($phone){
        ...
        $this->db->where('phone', $phone);
        $result['data']['isUpdate'] = $this->db->update('users',array('jifen'=>intval($tmp[0]['jifen']) +2000));
    ...
    **/
    public function exChangeCode(){
        ...
        if(empty($code)){
        	$this->json_arr['data']['state'] = '001';
            $this->json_arr['data']['msg'] = '数据不能为空！';
            echo json_encode($this->json_arr);
            return;
        }
        ...
        if($result){
            ...
            if($excode['sn'] == ""){
            	$this->json_arr['data']['state'] = '001';
                $this->json_arr['data']['msg'] = '兑换码获取失败!';
                echo json_encode($this->json_arr);
                return;
            }
            ...
            $this->json_arr['data']['state'] = '000';
            $this->eventDB->where('sn', $excode['sn']);
            $this->eventDB->update('560event', $updata);
            $this->json_arr['data']['excode'] = $excode['sn'];
            $excode = $excode['sn'];
            $message = "秀域体验码: {$excode} 请妥善保存！请于2015年8月31日前预约后到店出示验证码体验，秀域全国预约电话95026，秀域门店信息可前往官网查询http://www.showyu.com/";
            $this->tools->sendMessage($result['tel'],$message);
            $this->json_arr['data']['msg'] = $message;
            echo json_encode($this->json_arr);
            return;

        }else{
        	$this->json_arr['data']['state'] = '001';
            $this->json_arr['data']['msg'] = '兑换码异常！';
            echo json_encode($this->json_arr);
            return;
        }
...
点击这里直接问吧~ ';
                        ...
                        if($tmp[0]['banned_zixun'] == 1){
                            ...
                            if (($updata_session['login_num'] % 5) == 0) {
                                $result['zixun'] = '看看昆凌整了那么多，才收服了杰伦。
有什么整形美容的问题，想问全国顶级的专家吗？
点击这里直接问吧~ ';
                            } else {
...
//testing
	...
	function getinfo($param) {
		...
		$uid = $this->uid;
		$tmp = $this->db->query("SELECT users.ref_id,users.id,users.jifen,users.grade, users.username ,users.alias,users.email,users.phone,users.created,user_profile.* FROM users LEFT JOIN user_profile ON user_profile.user_id = users.id WHERE users.id = {$uid} LIMIT 1")->result_array();
		$result['data'] = $tmp[0];
    ...
    public function getSettingInfo($params = ''){
        ...
        if($this->uid){
            ...
            $zanMyNotReadCount = $this->db->query("select *from wen_zan where is_read = 0 and touid={$this->uid}")->num_rows();
            ...
            $zixunNotReadCount =$this->db->query("select *from wen_questions where is_read = 0 and fUid={$this->uid}")->num_rows();
    ...
    function DarenDetail($param = ''){
        ...
        $rs = $this->db->query('select id,username, alias, daren From users where users.daren=1 order by (select count(*) from wen_weibo where users.id=wen_weibo.uid) desc limit 20')->result_array();
        $daren = array();
        if(!empty($rs)){
            foreach($rs as $item){
                $item['alias'] = $item['alias'] ? $item['alias']:$item['username'];
                ...
                if(!empty($tmp)){
                    $ii = 0;
                    foreach($tmp as $i){
                        $item['note'][$ii]['img'] = $this->remote->show320($i['imgurl']);
	...
	*/
    ...
    public function updatePhone($param = ''){
        ...
        if($this->uid){
            ...
            if(!$this->isphone($phone, $this->uid)) {
                $this->db->where('id', $this->uid);
    ...
    function updateUserInfo($param = '') {
        ...
        if ($this->uid) {
            ...
            if(!empty($data['alias']) && !isset($_FILES['attachPic']['tmp_name']) && !empty($data['sex']) && !empty($data['city'])){
                $this->db->where('id',$uid);
                $this->db->update('users',array('jifen'=>500));
            }
        }else{
            $result['notice'] = '登录失败！';
            $result['state'] = '012';
			...
			if (true || $this->session->userdata('veryCodeTime') > time()) {
				...
				if ($this->input->get('type')) {
					if (!$this->_check_phone_no($phone)) {
						$message = '[编号:' . $sn . ']您的手机验证码是：' . $code . '退订回复0000';
						$time = time() + 120;
						$result['vcode'] = $code;
