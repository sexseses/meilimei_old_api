        ...
        if($nid > 0 ){
            ...
            if(!empty($rs)){
                ...
                foreach($rs as $item){
                    ...
                    $item['age'] = $this->getAge($temp[0]['age']);
                    if(preg_match("/^1[0-9]{10}$/",$item['username'])){
                        $item['username'] = substr($item['username'],0,4).'****';
                    }
                    $item['created_at'] = date('Y-m-d',$item['created_at']);
    ...
    public function saveUserDiary($param = ''){
        ...
        if(!empty($content) && $this->uid){
            if($type != 1) {
                ...
                if($doctor){
                    $result['data']['score'] =  $this->Score_model->addScore(51,$this->uid);
                }else if($hospital){
                    $result['data']['score'] = $this->Score_model->addScore(50,$this->uid);
                }
                $result['data']['score'] = $this->Score_model->addScore(49,$this->uid);
            ...
            if(!empty($tmp)){
                ...
                foreach($tmp as $item){
                    ...
                    if(preg_match("/^1[0-9]{10}$/",$item['fromusername'])){
                        $item['fromusername'] = substr($item['fromusername'],0,4).'****';
                    }
                    $item ['basicinfo'] = $this->getBasicInfo($temp[0]);
                    $item['level'] = $this->getLevel($temp[0]['jifen']);
                    $item['age'] = $this->getAge($temp[0]['age']);
                            ...
                            foreach($item['reply'] as $it){
                                ...
                                if(empty($it['imgfile'])) {
                                    ...
                                    if(preg_match("/^1[0-9]{10}$/",$it['fromusername'])){
                                        $it['fromusername'] = substr($it['fromusername'],0,4).'****';
                                    }
    ...
    */
    ...
    public function addNoteCategory($param = ''){
        ...
        if($title && $this->uid){
            ...
            if($imgurl != '') {
                $data = array('uid' => $this->uid, 'is' => $is, 'title' => $title, 'desc' => $desc, 'imgurl' => $imgurl,'rightimgurl'=> $rightimgurl, 'leftimgurl'=> $leftimgurl,'operation_time'=>$operation_time, 'created_at' => time(), 'updated_at' => time());
            }else{
                ...
                if($leftimgurl){
                    $data['leftimgurl'] = $leftimgurl;
                }else{
                    $data['leftimgurl'] = '';
                }

                if($rightimgurl){
                    $data['rightimgurl'] = $rightimgurl;
                }else{
                    $data['rightimgurl'] = '';
                }
            }
            ...
            if($type == 1) {
                ...
                if(intval($ncid) > 0){
                    ...
                    $rstemp = $this->Diary_model->getNoteCategoryc($ncid);
                    $arr_tmp = array();
        ...
        if($this->uid){
            ...
            if(!empty($temp)){
                foreach($temp as $item){
                    ...
                    $item['signin'] = $this->db->query("select *From user_signin where uid={$this->uid} and calDate='".date('Y-m-d')."'")->num_rows();
                    ...
                    $item['userBackgroundImg'] = $this->remote->getLocalImage($this->Background_Model->getUserBackground($this->uid,1));
                    unset($item['alias']);
                    unset($item['city']);
                    $result['data'] = $item;
                }
            ...
            if(1){
                ...
                if(!empty($tmp)){
                    ...
                    foreach($tmp as $item){
                        ...
                        $rs = $this->Diary_model->getDiaryDetail($item['nid']);
                        if(isset($rs[0]['imgurl'])&& $rs[0]['imgurl']){
                            if(empty($rs[0]['imgfile'])) {
                                $item['pic'] = $this->remote->getLocalImage($rs[0]['imgurl']);
                            }else{
                                $item['pic'] = $this->remote->getQiniuImage($rs[0]['imgfile']);
                ...
                if(!empty($tmp)){
                    foreach($tmp as $item){
                        ...
                        if(empty($item['tocontent'])){
                            ...
                            if(isset($r['pic']['savepath'])&& $r['pic']['savepath']){
                                if(empty($item['imgfile'])) {
                                    $item['pic'] = $this->remote->getLocalImage($r['pic']['savepath']);
                                }else {
                                    $item['pic'] = $this->remote->getQiniuImage($item['imgfile']);
    ...
    public function diaryList($pageSize=5) {
        ...
        $tmp = $this->Diary_model->getDiaryFrontList($offset,0);
