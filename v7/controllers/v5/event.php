        ...
        if(intval($event_id) > 0){
            ...
            if(!empty($tmp)){
                foreach($tmp as $item){
                    ...
                    if(strpos($item['event_content'],'img src="') !=0) {

                        $item['event_content'] = $this->gdetail(str_replace('img src="','img src="http://www.meilimei.com',$item['event_content']), 'tmp');
                    }else{
                    ...
                    $item['share_pic'] = $this->remote->show(str_replace('upload/','',$item['share_pic']));
