...
class topic extends MY_Controller {
				...
				if (!empty ($tmp)) {
					...
					if ($tmp[0]['type'] == 1 and !empty ($ptmp)) {
						if(empty($tmp[0]['imgfile'])) {
                            ...
                            $result['data']['url'] = $this->fileUrl.$ptmp[0]['imgfile'];
                        }
					}
					elseif ($tmp[0]['type'] == 8) {
					if(empty($tmp[0]['imgfile'])) {
	...
	public function Plist($id, $_width=360) {
		...
		foreach ($res as $r) {
            ...
            if(empty($r['info'])) {
                $r['info'] = !empty($r['points'][0]['remark']) && isset($r['points'][0]['remark']) ? $r['points'][0]['remark']: '';
            }else if(is_null($r['info'])){
                $r['info'] = '';
            }
            ...
            if(substr($r['savepath'],strlen($r['savepath'])-4) == '.mp4'){
                ...
                if (intval($arr_url[1]) >= 3 && intval(date('Y')) <= $arr_url[0]) {
                    ...
                    if (isset($arr_url[1])) {
                        ...
                        $url = str_replace('/' . $arr_url[1] . '/', '/' . $arr_url[1] . 'x'.$_width.'/', $r['savepath']);
                    }

                    //echo $this->remote->show320($url, $width);
                    $r['savepath'] = $this->remote->show800($url, $_width);
                    $r['vedio'] = '';
                } else {
                    if (isset($arr_url[1])) {
                    ...
                    $r['savepath'] = $this->remote->show320($url, $_width);
                    $r['vedio'] = '';
                }
            }
            $rt[] = $r;
	...
	public function topicSearch($param = '') {
		...
		if ($this->input->get('page') and $kw = mysql_real_escape_string(trim($this->input->get('kw')))) {
			...
			$this->db->select('users.jifen, users.age, users.city , users.alias as showname,wen_weibo.tags as tags,users.phone,wen_weibo.hot, wen_weibo.event_id,wen_weibo.hot_start,wen_weibo.hot_end,wen_weibo.top,wen_weibo.top_start,wen_weibo.top_end,wen_weibo.chosen,wen_weibo.chosen_start,wen_weibo.chosen_end,wen_weibo.weibo_id,wen_weibo.views,wen_weibo.comments as commentnums,wen_weibo.uid,wen_weibo.uid,wen_weibo.content,wen_weibo.ctime, wen_weibo.type_data');
			$this->db->from('wen_weibo');
    ...
    public function getItemDiaryList($item_name, $pageSize=1) {
        ...
        if(!empty($tmp)){
            ...
            foreach($tmp as $item){
                ...
                if(!empty($category)){
                    $item['operation_imgurl'] = $this->remote->show320($category[0]['imgurl']);
                }else{
                    $item['operation_imgurl'] = $this->remote->show320($item['imgurl']);
                }
                if(!empty($tmp)){
                    foreach($tmp as $i){
