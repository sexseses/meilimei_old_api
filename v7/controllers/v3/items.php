...
class items extends MY_Controller {
    ...
    function cItems($param = '') {
        ...
        foreach ($tmp as $r) {
            $r['burl'] = $this->remote->getLocalImage($r['burl']);
            $r['surl'] = $this->remote->getLocalImage($r['surl']);
            $result['data'][] = $r;
        }
    ...
    private function getHot($pid = 261){
        ...
        foreach($tmp as $key=>$item){
            $item['burl'] = $this->remote->getLocalImage($item['burl']);
            $item['surl'] = $this->remote->getLocalImage($item['surl']);
    ...
    private function getChildItem($pid = 0,$uid=0){
        ...
        foreach($tmp as $key=>$item){
            $item['burl'] = $this->remote->getLocalImage($item['burl']);
            $item['surl'] = $this->remote->shgetLocalImageow($item['surl']);
            ...
            foreach($itemtmp as $k=>$i){
                ...
                $itemtmp[$k]['burl'] = $this->remote->getLocalImage($itemtmp[$k]['burl']);
                $itemtmp[$k]['surl'] = $this->remote->getLocalImage($itemtmp[$k]['surl']);
                $tmpitem = $itemtmp;
            }
    ...
    function getIMtemInfo($param = '') {
        ...
        if ($id = intval($this->input->get('id'))) {
            ...
            if($tmp[0]['surl']) {
                $result['iteminfo']['surl'] = $this->remote->getLocalImage($tmp[0]['surl']);
                $result['iteminfo']['burl'] = $this->remote->getLocalImage($tmp[0]['burl']);
            }else{
    ...
    public function getItemInfo(){
        ...
        if ($id OR ($id = intval($this->input->get('id')))) {
            ...
            if(!empty($tmp)){
                foreach($tmp as $r){
                    if($r['img_s'] ) {
                        $r['surl'] = $this->remote->getLocalImage($r['img_s']);
                        $r['burl'] = $this->remote->getLocalImage($r['img_m']);
                    }else{
            ...
            if ($tags) {
                ...
                if (count($arr_tags) > 0) {
                    ...
                    if (!empty($res)) {
                        ...
                        foreach ($res as $item) {
                            $user_fav = array('cid' => $item['id'], 'uid' => $this->uid, 'tag_img' => $this->remote->getLocalImage($item['surl']),'colors'=>$item['colors'], 'tag' => $item['name'], 'created_at' => time(), 'updated_at' => time());
            ...
            if($tags) {
                ...
                if(count($arr_tags) > 0) {
                    ...
                    if (!empty($res)) {
                        ...
                        foreach ($res as $item) {
                            $user_fav = array('cid' => $item['id'], 'uid' => $this->uid, 'tag_img' => $this->remote->getLocalImage($item['surl']), 'tag' => $item['name'], 'colors'=>$item['colors'],'created_at' => time(), 'updated_at' => time());
