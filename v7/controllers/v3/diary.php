            ...
            if(!empty($tmpDayDiaryList)){
                ...
                foreach($tmpDayDiaryList as $k=>$item) {
                    ...
                    foreach ($tmp as $k => $citem) {
                        ...
                        $citem['sImgUrl'] = $this->remote->getLocalImage($citem['imgurl'], 80);
                        $citem['imgurl'] = $this->remote->getLocalImage($citem['imgurl']);
        ...
        if($item_name || $doctor || $yiyuan){
            ...
            if(!empty($result['data'])){
                ...
                foreach($result['data'] as $key=>$item){
                    ...
                    $item['sImgUrl'] = $this->remote->getLocalImage($item['imgurl'], 80);
                    $item['imgurl'] = $this->remote->getLocalImage($item['imgurl']);
                    unset($item['username']);
        ...
        if(intval($nid) > 0){
            ...
            if(!empty($tmp)){
                ...
                foreach($tmp as $item){
                    ...
                    $item ['basicinfo'] = $this->getBasicInfo($temp[0]);
                    $item['imgurl'] = isset($item['imgurl'])?$this->remote->getLocalImage($item['imgurl']):'';
                    ...
                    if($type == 1){
                        ...
                        if(!empty($item['reply'])){
                            ...
                            foreach($item['reply'] as $it){
                                $it['imgurl'] = isset($it['imgurl'])?$this->remote->getLocalImage($it['imgurl']):'';
