    ...
    private function getcomment($qid = 0, $uid = 0) {
        ...
        foreach ($tmp as $row) {
            ...
            if ($t = unserialize($row['data'])) {
                $row['pic'] = $this->remote->getLocalImage($t['linkpic']) ;
                $row['haspic'] = 1;
            }
            unset ($row['qid']);
            unset ($row['data']);
        ...
        if ($this->notlogin) {
            ...
            if ($qid = $this->input->post('qid') and ((isset ($_FILES['attachPic']['tmp_name']) and $_FILES['attachPic']['tmp_name']!='') OR $this->input->post('comment'))) {
                ...
                if (isset ($_FILES['attachPic']['tmp_name'])) {
                    ...
                    $returnSend['haspic'] = 1;
                    $returnSend['pic'] = $this->remote->getLocalImage($savepath);
    ...
    private function getqustions($uid){
        ...
        foreach($rmp as $r){
            ...
            if(isset($tmf['pic']['savepath'])){
                ...
                $r['pictures'][] = $this->remote->getLocalImage($tmf['pic']['savepath'],128);
            }
            $r['type_data'] = null;
            switch ($r['sex']) {
                case 1 :
        ...
        if ($uid = intval($this->input->get('uid'))) {
            ...
            foreach ($tmp as $row) {
                ...
                if(isset($tmps[1]['id'])){
                    ...
                    $r['savepath'] = $this->remote->getLocalImage($ptmp[0]['savepath'],128);
